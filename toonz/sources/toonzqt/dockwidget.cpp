...
// Il metodo QRectF::toRect pare che faccia la cosa commentata. Ovviamente non e' la cosa piu'
// simpatica - considera
// che in questo modo i bordi del rect di input *non vengono approssimati alle piu' vicine
// coordinate intere*!
...
DockWidget::DockWidget(QWidget *parent, Qt::WindowFlags flags)
	: QWidget(parent, flags), m_dragging(false), m_resizing(false), m_floating(true),
	  m_undocking(false), m_parentLayout(0), m_selectedPlace(0), m_maximized(0)
{
...
//! Returns true or false whether input \b QPoint is inside the dragging grip for \b this
//! DockWidget, or not.
//! Typically (and by default), true is returned whenever p lies inside the widget's title bar when
//! floating;
//! however, you are free to place the drag grip anywhere reimplementing this method in custom
//! dockwidgets.
	...
	if (m_resizing) {
		...
		if (distance > 8) {
			...
			if (m_parentLayout->undockItem(this)) {
				...
				// Re-grab mouse inputs. Seems that making the window float (i.e.: reparenting)
				// breaks old grab.
	...
	if (m_dragging) {
		if (m_selectedPlace) {
			DockPlaceholder *newSelected =
				(we->delta() > 0) ? m_selectedPlace->parentPlaceholder()
...
DockPlaceholder *DockWidget::placeAdjacentTo(DockWidget *dockWidget, int boundary)
{
	...
	if (((boundary == DockPlaceholder::left || boundary == DockPlaceholder::right) &&
		 r->getOrientation() == Region::horizontal) ||
		((boundary == DockPlaceholder::top || boundary == DockPlaceholder::bottom) &&
...
inline void DockPlaceholder::buildGeometry()
{
	...
	if (m_separator)
		...
		int margin =
			6; // layout->margin();   //Purtroppo questa info e' assegnata prima delle Room...
...
DockPlaceholder::DockPlaceholder(DockWidget *owner, Region *r, int idx, int attributes)
	: QWidget(owner), m_owner(owner), m_separator(0), m_region(r), m_idx(idx),
...
DockPlaceholder *DockPlaceholder::childPlaceholder(QPoint p)
{
	...
	return r->placeholders().size()
			   ? lastExtremity ? r->placeholders().back() : r->placeholders().front()
			   : this;
}
		...
		if (m_orientation == Region::horizontal) {
			...
			if (dx) {
				...
				for (i = m_index; dx != 0 && i >= 0; --i) {
					...
					newWidth =
						std::min(std::max(newWidth, (double)r->getMinimumSize(Region::horizontal)),
			...
			if (dy) {
				...
				for (i = m_index; dy != 0 && i >= 0; --i) {
					...
					newHeight =
						std::min(std::max(newHeight, (double)r->getMinimumSize(Region::vertical)),
