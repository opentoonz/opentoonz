...
void FlipConsole::enableButton(UINT button, bool enable, bool doShowHide)
{
	...
	case eGRed:
		...
		CASE eGGreen : if (m_doubleGreen) m_doubleGreen->setEnabledSecondButton(enable);
		CASE eGBlue : if (m_doubleBlue) m_doubleBlue->setEnabledSecondButton(enable);
		break;
	}
}
...
void FlipConsole::doButtonPressed(UINT button)
{
	...
	case eFirst:
		...
		CASE ePlay : __OR eLoop:
		{
			...
			if (!linked) {
				// if the play button pressed at the end frame, then go back to the start frame and
				// play
