...
//  * Il ricalcolo delle extremal sizes e' inutile nei resize events... Cerca di tagliare cose di
//  questo tipo - ottimizza!
//      => Pero' e' sensato in generale che redistribute possa ricalcolarle - forse si potrebbe fare
//      una redistribute
//         con un bool di ingresso. Se l'operazione contenitrice non puo' cambiare le extremal
//         sizes, allora metti false..
//  * Implementa gli stretch factors

//  * Nascondere le finestre dockate...? Molto rognoso...

//  * Una dock widget potrebbe non avere i soli stati docked e floating... Che succede se e' una
//  subwindow
//    non dockata in un dockLayout??
//  * Spezzare tdockwindows.h in tmainwindow.h e tdockwidget.h, come in Qt?
//  * Muovere o fare operazioni su una dockWidget allo stato attuale non e' sicuro se quella non e'
//  assegnata ad
//    un DockLayout!! Comunque, si puo' assumere che il parente di una DockWidget non sia nemmeno
//    una QWidget
//    che implementa un DockLayout. Questa cosa puo' venire meno in un'implementazione specifica
//    come TDockWidget?
//      Esempio: vedi calculateDockPlaceholders ad un drag, viene lanciato comunque...
//  * Ha senso mettere DockLayout e DockWidget nella DVAPI? Forse se definissi delle inline
//  opportune in TDockWidget...
//  * Quanto contenuto in DockSeparator::mousePress e mouseMove dovrebbe essere reso pubblico. Anche
//  la geometria
//    delle regioni potrebbe essere editabile dall'utente... ma prima si dovrebbero fare gli stretch
//    factors!!
//      > Ossia: se si vuole esplicitamente settare la geometria di una regione, si potrebbe fare
//      che quella
...
//  * Capita spesso di considerare l'ipotetica nuova radice della struttura. Perche' non metterla
//  direttamente??

//  X Non e' possibile coprire tutte le possibilita' di docking con il sistema attuale, anche se e'
//  comunque piu'
...
DockLayout::DockLayout() : m_maximizedDock(0), m_decoAllocator(new DockDecoAllocator())
{
}
...
inline void DockLayout::updateSeparatorCursors()
{
	...
	for (i = 0; i < m_regions.size(); ++i) {
		...
		// NOTE: If this update follows only from a dock/undock, this should be disabled 'til
		// 'Otherwise'
		QSize size = toRect(r->getGeometry()).size();
		bool isExtremeSize = (orientation == Region::horizontal)
		...
		Qt::CursorShape shape =
			(orientation == Region::horizontal) ? Qt::SplitHCursor : Qt::SplitVCursor;
		for (; jInt < k; ++jInt)
		...
		if (m_regions[i]->getItem()) {
			...
			for (j = 0; j < sepList.size(); ++j) {
				...
				if (r->getOrientation() == Region::horizontal) {
					...
					sepList[j]->setGeometry(
						QRect(leftAdjRect.topRight(), QSize(spacing(), leftAdjRect.height())));
					sepList[j]->m_index = j;
				} else {
					leftAdjRect.adjust(0, 0, 0, 1);
					sepList[j]->setGeometry(
						QRect(leftAdjRect.bottomLeft(), QSize(leftAdjRect.width(), spacing())));
					sepList[j]->m_index = j;
				}
			}
		}
...
void DockLayout::redistribute()
{
	if (!m_regions.empty()) {
		...
		// NOTA: Sarebbe da fare solo se un certo flag lo richiede; altrimenti tipo per resize
		// events e' inutile...
		...
		// Always check main window consistency before effective redistribution. DO NOT ERASE or
		// crashes may occur...
...
void DockLayout::calculateDockPlaceholders(DockWidget *item)
{
	...
	if (!m_regions.size()) {
		if (isPossibleInsertion(item, 0, 0)) {
			...
			item->m_placeholders.push_back(
				item->m_decoAllocator->newPlaceBuilt(item, 0, 0, DockPlaceholder::root));
			return;
		}
	}
	...
	if (isPossibleInsertion(item, 0, 0)) {
		...
		if (m_regions.front()->getOrientation() == Region::horizontal) {
			item->m_placeholders.push_back(
				item->m_decoAllocator->newPlaceBuilt(item, 0, 0, DockPlaceholder::top));
			item->m_placeholders.push_back(
				item->m_decoAllocator->newPlaceBuilt(item, 0, 1, DockPlaceholder::bottom));
		} else {
			item->m_placeholders.push_back(
				item->m_decoAllocator->newPlaceBuilt(item, 0, 0, DockPlaceholder::left));
			item->m_placeholders.push_back(
				item->m_decoAllocator->newPlaceBuilt(item, 0, 1, DockPlaceholder::right));
		}
	}
	...
	for (i = 0; i < m_regions.size(); ++i) {
		...
		if (isPossibleInsertion(item, r, 0)) {
			...
			if (r->getOrientation() == Region::horizontal) {
				// Left side
				item->m_placeholders.push_back(
					item->m_decoAllocator->newPlaceBuilt(item, r, 0, DockPlaceholder::left));
				r->m_placeholders.push_back(item->m_placeholders.back());

				// Separators
				for (j = 1; j < r->getChildList().size(); ++j) {
					item->m_placeholders.push_back(
						item->m_decoAllocator->newPlaceBuilt(item, r, j, DockPlaceholder::sepVert));
					r->m_placeholders.push_back(item->m_placeholders.back());
				}

				// Right side
				item->m_placeholders.push_back(
					item->m_decoAllocator->newPlaceBuilt(item, r, j, DockPlaceholder::right));
				r->m_placeholders.push_back(item->m_placeholders.back());
			} else {
				// Top side
				item->m_placeholders.push_back(
					item->m_decoAllocator->newPlaceBuilt(item, r, 0, DockPlaceholder::top));
				r->m_placeholders.push_back(item->m_placeholders.back());

				for (j = 1; j < r->getChildList().size(); ++j) {
					item->m_placeholders.push_back(
						item->m_decoAllocator->newPlaceBuilt(item, r, j, DockPlaceholder::sepHor));
					r->m_placeholders.push_back(item->m_placeholders.back());
				}

				// Bottom side
				item->m_placeholders.push_back(
					item->m_decoAllocator->newPlaceBuilt(item, r, j, DockPlaceholder::bottom));
				r->m_placeholders.push_back(item->m_placeholders.back());
			}
		}
	}
...
//!\b NOTE: Window flags are resetted to floating appearance (thus hiding the widget). Since the
//!geometry
//! reference changes a geometry() update may be needed - so item's show() is not forced here. You
//! should
...
bool DockLayout::undockItem(DockWidget *item)
{
	...
	// NOTA: Usando la flag Qt::Window il focus viene automaticamente riassegnato, con un po' di
	// ritardo.
							 ...
							 std::vector<std::pair<int, int>> intervals, double sum)
{
	...
	// it can be demonstrated that at least one coordinate of the current partial solution is
	// related
	...
	for (i = 0; i < target.size(); ++i) {
		if (nearest[i] < intervals[i].first || nearest[i] > intervals[i].second) {
			distance = nearest[i] < intervals[i].first ? intervals[i].first - nearest[i]
...
void Region::redistribute()
{
	...
	// If there is no need to expand this region, maintain current geometry; otherwise, expand at
	// minimum.
	...
	if (m_parent != 0)
		regionSize[m_orientation] =
			std::min((double)m_parent->m_maximumSize[m_orientation], regionSize[m_orientation]);
	...
	for (i = 0; i < m_childList.size(); ++i) {
		...
		targetSizes[i] = (m_orientation == Region::horizontal) ? m_childList[i]->m_rect.width()
...
//! Calculates minimum occupiable space in \b this region on given \b direction. Also stores cache
//! for it.
int Region::calculateMinimumSize(bool direction, bool recalcChildren)
{
	...
	if (m_item) {
		sumMinSizes = maxMinSizes = (direction == horizontal) ? m_item->minimumSize().width()
...
//! Calculates maximum occupiable space in \b this region on given \b direction. Also stores cache
//! for it.
...
int Region::calculateMaximumSize(bool direction, bool recalcChildren)
{
	...
	if (m_item) {
		sumMaxSizes = minMaxSizes = (direction == horizontal) ? m_item->maximumSize().width()
...
inline bool Region::addItemSize(DockWidget *item)
{
	...
	if (m_orientation == horizontal) {
		...
		m_minimumSize[vertical] =
			std::max(m_minimumSize[vertical], item->getDockedMinimumSize().height());
		m_maximumSize[vertical] =
			std::min(m_maximumSize[vertical], item->getDockedMaximumSize().height());
	} else {
		// Viceversa
		...
		m_minimumSize[horizontal] =
			std::max(m_minimumSize[horizontal], item->getDockedMinimumSize().width());
		m_maximumSize[horizontal] =
			std::min(m_maximumSize[horizontal], item->getDockedMaximumSize().width());
	}
	...
	// Now, climb parent hierarchy and update extremal sizes. If minSizes get > maxSizes, return
	// failed insertion
	Region *r = m_parent;
	while (r) {
	...
	if (m_orientation == horizontal) {
		...
		for (i = 0; i < m_childList.size(); ++i)
			if (m_childList[i]->getItem() != item) {
				m_minimumSize[vertical] =
					std::max(m_minimumSize[vertical], m_childList[i]->getMinimumSize(vertical));
				m_maximumSize[vertical] =
					std::min(m_maximumSize[vertical], m_childList[i]->getMaximumSize(vertical));
			}
	} else {
		// Viceversa
		...
		for (i = 0; i < m_childList.size(); ++i)
			if (m_childList[i]->getItem() != item) {
				m_minimumSize[horizontal] =
					std::max(m_minimumSize[horizontal], m_childList[i]->getMinimumSize(horizontal));
				m_maximumSize[horizontal] =
					std::min(m_maximumSize[horizontal], m_childList[i]->getMaximumSize(horizontal));
			}
	}
...
inline bool DockLayout::isPossibleInsertion(DockWidget *item, Region *parentRegion,
											int insertionIdx)
{
	const int inf = 1000000;
	...
	if (m_regions.size()) {
		...
		{
			// Insertion under new root: simulated by adding with m_regions.front() on its opposite
			// direction;
	...
	if (rootMinSize.width() > mainWindowWidth || rootMinSize.height() > mainWindowHeight ||
		rootMaxSize.width() < mainWindowWidth || rootMaxSize.height() < mainWindowHeight) {
		result = false;
	}
...
inline DockSeparator *DockDecoAllocator::newSeparator(DockLayout *owner, bool orientation,
													  Region *parentRegion)
{
...
inline DockPlaceholder *DockDecoAllocator::newPlaceholder(DockWidget *owner, Region *r, int idx,
														  int attributes)
{
...
// BuildGeometry() method should not be called inside the base contructor - because it's a virtual
// method.
// So we provide this little inline...
inline DockPlaceholder *DockDecoAllocator::newPlaceBuilt(DockWidget *owner, Region *r, int idx,
														 int attributes)
{
