...
*/
DVAPI bool increaseControlPoints(TStroke &stroke,					 //!< Stroke to be deformed.
...
DVAPI void modifyControlPoints(TStroke &stroke, const TStrokeDeformation &deformer);

DVAPI void modifyControlPoints(TStroke &stroke, const TStrokeDeformation &deformer,
							   std::vector<double> &controlPointLen);

DVAPI void modifyThickness(TStroke &stroke, const TStrokeDeformation &deformer,
						   std::vector<double> &controlPointLen, bool exponentially = false);
...
*/
DVAPI void
transform_thickness(TStroke &stroke,	 //!< Stroke whose thickness will be transformed.
  ...
  \details  The conversion is actually a 2 step process. First, segment midpoints
			...
			The triplets conversion step allows the following control parameters:
			  ...
			  <LI> Polyline corners whose edges inner product is strictly above \a angleTol are
				   transformed into full corners. The range for \a angleTol should be <TT>[-1,
  1]</TT>.</LI>
...
DVAPI void polylineToQuadratics(
	...
	double relativeTol = 0.25, double relativeDistTol = 0.25, double mergeTol = 1.0);
