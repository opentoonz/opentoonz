...
class DVAPI TIStream
{
  ...
  */
	...
	TIStream &operator>>(
		TPersist *&v); //!< \a Allocates and reads a TPersist derived object data from the stream.
  ...
  */
	...
	TFilePath getFilePath(); //!< Returns the stream's path (i.e. the opened filename associated to
							 //!the input stream).
	...
	VersionNumber
	getVersion() const; //!< Returns the currently stored version of the opened document.
						//!  \sa setVersion()

	void setVersion(const VersionNumber
						&version); //!< Returns the currently stored version of the opened document.
								   //!  \sa setVersion()
								   /*!
...
template <class T> TIStream &operator>>(TIStream &is, T *&v)
{
	TPersist *persist = 0;
	is >> persist;
...
class DVAPI TOStream
{
  ...
  */
	...
	void openCloseChild(std::string tagName,
						const std::map<std::string, std::string>
							&attributes); //!< \brief Writes a tag <tagName attribute1="value1" ../>
										  //!  to the stream, opening a tag with embedded attributes
	...
	TFilePath
	getFilePath(); //!< Returns the file path of the file associated to this output stream.
	TFilePath getRepositoryPath(); //!< \deprecated
