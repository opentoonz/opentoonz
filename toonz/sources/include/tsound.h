...
class DVAPI TSoundTrackFormat
{
  public:
	...
	TSoundTrackFormat(TUINT32 sampleRate = 0, int bitPerSample = 0, int channelCount = 0,
					  bool signedSample = true)
		: m_sampleRate(sampleRate), m_bitPerSample(bitPerSample), m_channelCount(channelCount),
...
class DVAPI TSoundTrack : public TSmartObject
{
  ...
  protected:
	...
	TSoundTrack(TUINT32 sampleRate, int bitPerSample, int channelCount, int sampleSize,
  ...
  */
	static TSoundTrackP create(TUINT32 sampleRate, int bitPerSample, int channelCount,
							   TINT32 sampleCount, bool signedSample = true);

	static TSoundTrackP create(TUINT32 sampleRate, int bitPerSample, int channelCount,
							   TINT32 sampleCount, void *buffer, bool signedSample = true);
  ...
  */
	...
	virtual void getMinMaxPressure(TINT32 s0, TINT32 s1, TSound::Channel chan, double &min,
								   double &max) const = 0;
  ...
  Range in seconds
  */
	void getMinMaxPressure(double t0, double t1, TSound::Channel chan, double &min,
						   double &max) const;
...
class TSoundDeviceException : public TException
{
  ...
  public:
	...
	TSoundDeviceException(Type type, const std::string &msg) : TException(msg), m_type(type) {}
	TSoundDeviceException(Type type, const std::wstring &msg) : TException(msg), m_type(type) {}
