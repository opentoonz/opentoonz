...
/*! \note The input and output rasters must have the same size and pixel type.
			Supports rout == rin. */
DVAPI void rgbmScale(TRasterP rout, TRasterP rin, const double *k, const double *a, const int *out0,
					 const int *out1);

DVAPI void rgbmScale(TRasterP rout, TRasterP rin, double kr, double kg, double kb, double km,
...
/*! \note The input and output rasters must have the same size and pixel type.
			Supports rout == rin. */
DVAPI void rgbmAdjust(TRasterP rout, TRasterP rin, const int *in0, const int *in1, const int *out0,
					  const int *out1);
...
DVAPI void checkBoard(TRasterP rout, const TPixel32 &, const TPixel32 &, const TDimensionD &,
					  const TPointD &);

//! Make the image over of the \b up raster on the \b dn raster and put the resulting raster in \b
//! rout
...
DVAPI void over(const TRasterP &out, const TRasterP &up, const TPoint &pos, const TAffine &aff,
				ResampleFilterType filterType = Triangle);
//! Make the image over of the \b up raster on the \b dn raster using a \b mask and put the
...
DVAPI void quickPut(const TRasterP &out, const TRasterP &up, const TAffine &aff,
					const TPixel32 &colorScale = TPixel::Black, bool doPremultiply = false,
					bool whiteTransp = false, bool firstColumn = false,
...
DVAPI void quickPut(const TRasterP &out, const TRasterCM32P &up, const TPaletteP &plt,
					const TAffine &aff, const TPixel32 &globalColorScale = TPixel::Black,
					bool inksOnly = false);
...
{
  public:
	TPixel32 m_globalColorScale, m_transpCheckPaint, m_transpCheckBg, m_transpCheckInk;
	...
	CmappedQuickputSettings()
		: m_globalColorScale(TPixel32::Black), m_inksOnly(false), m_transparencyCheck(false),
		  m_blackBgCheck(false), m_inkIndex(-1), m_paintIndex(-1)
	{
	}
};

DVAPI void quickPut(const TRasterP &dn, const TRasterCM32P &upCM32, const TPaletteP &plt,
					const TAffine &aff, const CmappedQuickputSettings &settings);
...
DVAPI void invert(TRasterP ras, bool invRed = true, bool invGreen = true, bool invBlue = true,
				  bool invMatte = false);

//! Add \b rdown pixels to \b rup pixels in according with the factor \b v and put the resulting
//! raster in \b rout
...
DVAPI void mult(const TRasterP &rup, const TRasterP &rdown, const TRasterP &rout, int v,
				bool matte); //[-255..+255]
...
DVAPI void crossDissolve(const TRasterP &rup, const TRasterP &rdown, const TRasterP &rout,
						 UCHAR v); //[0..255]
...
//! Make a \b gamma correct in the \b raster
DVAPI void gammaCorrectRGBM(TRasterP raster, double gammar, double gammag, double gammab,
							double gammam);
...
DVAPI void blur(const TRasterP &dstRas, const TRasterP &srcRas, double blur, int dx, int dy,
				bool useSSE = false);
