 ...
 */
TNZAPI void create_raster_with_extra(RASTER *raster, int xsize, int ysize, RAS_TYPE type,
									 UCHAR extra_mask);
 ...
 */
TNZAPI int create_subraster(RASTER *rin, RASTER *rout, int x0, int y0, int x1, int y1);
...
TNZAPI void rop_subimage_to_raster(IMAGE *img, int img_x0, int img_y0, int img_x1, int img_y1,
 ...
 */
TNZAPI void rop_copy(RASTER *rin, RASTER *rout, int x1, int y1, int x2, int y2, int newx, int newy);
 ...
 */
TNZAPI void rop_copy_extra(RASTER *rin, RASTER *rout, int x1, int y1, int x2, int y2, int newx,
						   int newy);
 ...
 */
TNZAPI void rop_copy_90(RASTER *rin, RASTER *rout, int x1, int y1, int x2, int y2, int newx,
...
TNZAPI void rop_shrink(RASTER *rin, RASTER *rout, int x1, int y1, int x2, int y2, int newx,
					   int newy, int factor);
 ...
 */
TNZAPI void rop_zoom_out(RASTER *rin, RASTER *rout, int x1, int y1, int x2, int y2, int newx,
 ...
 */
TNZAPI void rop_zoom_out_90(RASTER *rin, RASTER *rout, int x1, int y1, int x2, int y2, int newx,
 ...
 */
TNZAPI void rop_ovr_rgbm(int r, int g, int b, int m, RASTER *rtop, RASTER *rovr);
 ...
 */
TNZAPI void rop_ovr_patches(RASTER *rbot, UCHAR botmask, RASTER *rtop, UCHAR topmask, RASTER *rovr);
 ...
 */
TNZAPI void rop_zoom_out_ovr_2(RASTER *rbot, RASTER *rtop, RASTER *rovr, int abs_zoom_level);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

/* like rop_ovr_3, but zooming out at the same time (see rop_zoom_out)
 */
TNZAPI void rop_zoom_out_ovr_3(RASTER *rbot, RASTER *rmid, RASTER *rtop, RASTER *rovr,
							   int abs_zoom_level);
 ...
 * if the bottom buffer is NIL, white is assumed
 */
TNZAPI void rop_zoom_out_onion(RASTER *rbot, RASTER *rtop, RASTER *rovr, int abs_zoom_level);
 ...
 */
TNZAPI void rop_scale(RASTER *rin, RASTER *rout, double sx, double sy, double cxin, double cyin,
					  double cxout, double cyout, FLT_TYPE filtertype, double blur);
 ...
 * onto (cxout,cyout)
 */
TNZAPI void rop_rotate(RASTER *rin, RASTER *rout, double angle, double cxin, double cyin,
					   double cxout, double cyout);

TNZAPI void rop_rotate_sin(RASTER *rin, RASTER *rout, double sin, double cos, double cxin,
						   double cyin, double cxout, double cyout);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void get_flt_fun_rad(FLT_TYPE flt_type, double (**flt_fun)(double), double *flt_rad);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_resample(RASTER *rin, RASTER *rout, AFFINE aff, FLT_TYPE flt_type, double blur);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_resample_tcm(RASTER *rin, RASTER *rout, AFFINE aff, FLT_TYPE flt_type, double blur,
							 TCM_INFO tcm_info);
...
TNZAPI void rop_fill_cmap_ramp(RASTER *ras, TCM_INFO info, LPIXEL color, LPIXEL pencil,
							   int color_index, int pencil_index, int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_fill_cmap_colramp(RASTER *ras, TCM_INFO info, LPIXEL color, int color_index,
								  int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_fill_cmap_penramp(RASTER *ras, TCM_INFO info, LPIXEL pencil, int pencil_index,
								  int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_custom_fill_cmap_ramp(RASTER *ras, TCM_INFO info, LPIXEL color, LPIXEL pencil,
									  int color_index, int pencil_index, int already_premultiplied,
									  int *custom_tone);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_custom_fill_cmap_colramp(RASTER *ras, TCM_INFO info, LPIXEL color, int color_index,
										 int already_premultiplied, int *custom_tone);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_custom_fill_cmap_penramp(RASTER *ras, TCM_INFO info, LPIXEL pencil,
										 int pencil_index, int already_premultiplied,
										 int *custom_tone);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_fill_cmap_buffer(RASTER *ras, TCM_INFO info, LPIXEL *color, LPIXEL *pencil,
								 int already_premultiplied);
...
TNZAPI void rop_custom_fill_cmap_buffer(RASTER *ras, TCM_INFO info, LPIXEL *color, LPIXEL *pencil,
										int already_premultiplied, int *custom_tone);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_custom_fill_cmap_colbuffer(RASTER *ras, TCM_INFO info, LPIXEL *color,
										   int already_premultiplied, int *custom_tone);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_custom_fill_cmap_penbuffer(RASTER *ras, TCM_INFO info, LPIXEL *pencil,
										   int already_premultiplied, int *custom_tone);
