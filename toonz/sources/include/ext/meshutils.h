...
DVAPI void transform(const TMeshImageP &image, //!< Mesh image to be transformed.
...
DVAPI void tglDrawEdges(const TMeshImage &image, //!< Input mesh image whose edges will be drawn.
...
DVAPI void tglDrawFaces(const TMeshImage &image, //!< Input mesh image whose faces will be drawn.
						const PlasticDeformerDataGroup *deformerDatas =
							0 //!< Optional data about a deformation of the input image.
						);
...
DVAPI void tglDrawRigidity(
	...
	double minColor[4], //!< RGBM color quadruple (in [0, 1]) corresponding to the lowest rigidity
						//!value.
...
DVAPI void tglDraw(const TMeshImage &image,			   //!< Mesh image to be drawn.
