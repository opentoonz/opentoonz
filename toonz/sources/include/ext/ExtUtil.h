...
/**
   ...
   */
DVAPI bool straightCornersDetector(const TStroke *stroke, std::vector<double> &corners);
...
/**
   ...
   */
DVAPI bool detectStraightIntervals(const TStroke *stroke, ToonzExt::Intervals &intervals,
...
/**
   ...
   */
DVAPI bool findNearestStraightCorners(const TStroke *s, double actualW, Interval &out,
...
/**
   ...
   */
DVAPI bool cornersDetector(const TStroke *stroke, int minDegree, std::vector<double> &corners);
...
/**
   ...
   * These intervals are stored in an @c vector od Interval.
   */
DVAPI bool detectSpireIntervals(const TStroke *stroke, ToonzExt::Intervals &intervals,
								int minDegree);

/**
   * @brief Retrieve parameters for corners near actualW parameter.
   */
DVAPI bool findNearestSpireCorners(const TStroke *s, double actualW, Interval &out, int cornerSize,
								   const ToonzExt::Intervals *const cl = 0,
								   double tolerance = TConsts::epsilon);
...
/**
   ...
   */
DVAPI void findCorners(const TStroke *stroke, Intervals &corners, Intervals &intervals, int angle,
					   double tolerance);
...
DVAPI bool findNearestCorners(const TStroke *stroke, double w, Interval &out,
							  const Intervals &values, double tolerance = TConsts::epsilon);
...
/**
   ...
   */
DVAPI TStroke *rotateControlPoint(const TStroke *stroke, const ToonzExt::EvenInt &even,
								  double atLenght);
...
/**
   ...
   */
DVAPI bool replaceStroke(TStroke *old_stroke, TStroke *new_stroke, unsigned int n_,
						 TVectorImageP &vi);
...
template <class T> inline bool isValid(const T *s)
{
	assert(0 != s);
	if (!s)
		return false;
