...
class DVAPI MeshTexturizer
{
  ...
  public:
	...
	int
	bindTexture(const TRaster32P &ras,
				const TRectD &geometry, //!< Moves a \a copy of the supplied raster image to VRAM,
				PremultMode premultiplyMode = NONPREMULTIPLIED); //!  and assigns an identification
																 //!  number to it.
																 //!  \param ras  Texture to be
																 //!  copied to VRAM.  \param
																 //!  geometry  The texture's
																 //!  geometry in the world - with
																 //!  respect to compiled meshes.
																 //!  \param premultiplyMode
																 //!  Specified whether ras is to be
																 //!  intended premultiplied or not.

	/*! \warning  The pointer returned by getTextureData() is <B>owned by this class</B> -
				it must not be deleted.                                                       */

	TextureData *
	getTextureData(int textureId); //!< Retrieves the texture data built from a previously
	//!  bound texture image.                                       \param textureId  Identifier of
	//!  the texture to be retrieved  \returns See summary.
	...
	void rebindTexture(int textureId, const TRaster32P &ras,
					   const TRectD &geometry, //!< Rebinds the image bound to the specified texture
					   PremultMode premultiplyMode = NONPREMULTIPLIED); //!  id to a new image.
	void unbindTexture(int textureId); //!< Deletes the texture associated to the passed id.
									   //!\param textureId  Identifier of the texture to be unbound.
