...
class DVAPI TStroke : public TSmartObject
{
  ...
  public:
	 ...
	 */
	bool getNearestChunk(const TPointD &p, double &outT, int &chunkIndex, double &distance2,
	...
	/*!
	  \finds all points on stroke which are "enough" close to point p. return the number of such
	  points.
	 */
	int getNearChunks(const TThickPoint &p, std::vector<TThickPoint> &pointsOnStroke,
					  bool checkBBox = true) const;
	...
	*/
	 ...
	 */
	inline TPointD getPointAtLength(double s) const { return convert(getThickPointAtLength(s)); }
	...
	//! Gets the chunk and its parameter at stroke parameter \b w; returns true if a chunk was
	//! found.
	...
	*/
	static TStroke *interpolate(const std::vector<TThickPoint> &points, double error,
								bool findCorners = true);
 ...
 */
DVAPI int intersect(const TStroke *s1, const TStroke *s2, std::vector<DoublePair> &intersections,
					bool checkBBox = true);
...
DVAPI void computeQuadraticsFromCubic(const TThickPoint &p0, const TThickPoint &p1,
									  const TThickPoint &p2, const TThickPoint &p3, double error,
