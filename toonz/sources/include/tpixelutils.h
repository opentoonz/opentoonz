...
*/
template <class T> inline T blend(const T &a, const T &b, double t)
{
...
*/
template <class T> inline T blend(const T &a, const T &b, int num, int den)
{
	return T(
		(int)(((den - num) * a.r + num * b.r) / den), (int)(((den - num) * a.g + num * b.g) / den),
		(int)(((den - num) * a.b + num * b.b) / den), (int)(((den - num) * a.m + num * b.m) / den));
}

template <class T> inline T antialias(const T &a, int num)
{
	return T((int)((num * a.r) / 255), (int)((num * a.g) / 255), (int)((num * a.b) / 255),
			 (int)((num * a.m) / 255));
}
...
template <class T, class Q> DVAPI inline T overPixT(const T &bot, const T &top)
{
	...
	return T((r < max) ? (Q)r : (Q)max, (g < max) ? (Q)g : (Q)max, (b < max) ? (Q)b : (Q)max,
			 (bot.m == max) ? max : max - (max - bot.m) * (max - top.m) / max);
}

//-----------------------------------------------------------------------------
template <class T, class S, class Q> DVAPI inline T overPixGRT(const T &bot, const S &top)
{
	UINT max = T::maxChannelValue;
...
template <class T, class Q> DVAPI inline T quickOverPixDarkenBlendedT(const T &bot, const T &top)
{
	UINT max = T::maxChannelValue;
...
//-----------------------------------------------------------------------------
template <class T, class S, class Q> DVAPI inline T quickOverPixGRT(const T &bot, const S &top)
{
	UINT max = T::maxChannelValue;
...
template <class T, class Q> DVAPI inline void overPix(T &outPix, const T &bot, const T &top)
{
	UINT max = T::maxChannelValue;
...
DVAPI inline TPixel32 overPixOnWhite(const TPixel32 &top)
{
	...
	if (top.m == max)
		...
		return TPixel32(top.r + max - top.m, top.g + max - top.m, top.b + max - top.m, max);
}
...
{
	return TPixel64(static_cast<TPixel64::Channel>(pix.r * pix.m / 65535.0),
					static_cast<TPixel64::Channel>(pix.g * pix.m / 65535.0),
					static_cast<TPixel64::Channel>(pix.b * pix.m / 65535.0), pix.m);
}

DVAPI inline TPixel32 depremultiply(const TPixel32 &pix)
{
	return TPixel32(static_cast<TPixel32::Channel>(pix.r * 255.0 / pix.m),
					static_cast<TPixel32::Channel>(pix.g * 255.0 / pix.m),
					static_cast<TPixel32::Channel>(pix.b * 255.0 / pix.m), pix.m);
}

DVAPI inline TPixel64 depremultiply(const TPixel64 &pix)
{
	return TPixel64(static_cast<TPixel64::Channel>(pix.r * 65535.0 / pix.m),
					static_cast<TPixel64::Channel>(pix.g * 65535.0 / pix.m),
					static_cast<TPixel64::Channel>(pix.b * 65535.0 / pix.m), pix.m);
}
...
/*!
  ...
  OUT: r,g,b in [0..1]
*/
DVAPI void HSV2RGB(double hue, double sat, double value, double *red, double *green, double *blue);

//-----------------------------------------------------------------------------

DVAPI void rgb2hsv(int dstHsv[3], const TPixel32 &srcRgb, int maxHsv = 255);

DVAPI void RGB2HSV(double r, double g, double b, double *h, double *s, double *v);
...
DVAPI void HLS2RGB(double h, double l, double s, double *r, double *g, double *b);
...
DVAPI void rgb2hls(double r, double g, double b, double *h, double *l, double *s);
...
template <class T> class PixelConverter
{
  public:
...
template <> class PixelConverter<TPixel32>
{
  public:
...
template <> class PixelConverter<TPixel64>
{
  public:
...
template <> class PixelConverter<TPixelD>
{
  public:
...
template <class T> void add(T &pixout, const T &pixin, double v)
{
	TINT32 r, g, b, m;
...
template <class T> void sub(T &pixout, const T &pixin, double v)
{
	TINT32 r, g, b, m;
...
//! component on pixin.
template <class T> void mult(T &pixout, const T &pixin, double v)
{
	double r, g, b, m;
	r = pixin.r + v;
	g = pixin.g + v;
	b = pixin.b + v;
	m = pixin.m + v;
	pixout.r = (r < 0) ? 0 : ((r < T::maxChannelValue)
								  ? troundp(r * (pixout.r / (double)T::maxChannelValue))
								  : pixout.r);
	pixout.g = (g < 0) ? 0 : ((g < T::maxChannelValue)
								  ? troundp(g * (pixout.g / (double)T::maxChannelValue))
								  : pixout.g);
	pixout.b = (b < 0) ? 0 : ((b < T::maxChannelValue)
								  ? troundp(b * (pixout.b / (double)T::maxChannelValue))
								  : pixout.b);
	pixout.m = (m < 0) ? 0 : ((m < T::maxChannelValue)
...
template <class T> void lighten(T &pixout, const T &pixin, double v)
{
...
template <class T> void darken(T &pixout, const T &pixin, double v)
{
	pixout.r = pixin.r < pixout.r ? pixin.r : pixout.r;
	pixout.g = pixin.g < pixout.g ? pixin.g : pixout.g;
	pixout.b = pixin.b < pixout.b ? pixin.b : pixout.b;
