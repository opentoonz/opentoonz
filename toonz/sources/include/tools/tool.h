...
class DVAPI TMouseEvent
{
  ...
  public:
	...
	{
		m_modifiersMask =
			ModifierMask((shiftPressed << SHIFT_BITSHIFT) | (altPressed << ALT_BITSHIFT) |
...
/*!
 ...
 <UL>
			...
			Toonz enforces a strict classification of its tools that is used to enable or disable
them
			in the appropriate contexts:
<UL>
			  <LI> <B>Generic Tools:</B> the tool is always enabled, since it does not need to
access
					specific scene contents. Hidden tools typically prefer to select this type since
they
					should handle disablements silently.</LI>
			  <LI> <B>Column Tools:</B> the tool is used to alter or define the placement of a
column's
					content. It is disabled in Filmstrip view mode, since that's a strictly
level-related view.</LI>
			  <LI> <B>LevelRead Tools:</B> the tool is used to \a read a level's images data. It is
therefore
					enabled on all view modes. The tool is disabled in camera stand or 3D view modes
if the
					level's host column has sustained a placement which makes it impossible to
access image
					data (as is the case with Plastic-deformed columns).</LI>
			  <LI> <B>LevelWrite Tools:</B> the tool is used to \a write a level's images data. It
is
					disabled in all contexts where a LevelRead Tool would be disabled. It is also
disabled in
					case the current level is of a type unsupported for write, \a or the level is
read-only
					on disk.</LI>
</UL>
			Furthermore, tools define a bitwise combination of <I>Target Types</I>, which are the
category
 ...
 <UL>
			  ...
			  \par Drawing
			Tools use OpenGL to draw in their currently associated Viewer instance, which can be
retrieved
			through the getViewer() accessor function. The viewer is assigned to the tool just
before it
			invokes the tool's draw() function - use the onSetViewer() virtual method to access
viewer data
			\a before the viewer starts drawing (observe the tool is typically drawn as an overlay,
on top
			of other stuff).
 \n\n
			Just before draw() is invoked by the viewer, the \p GL_MODELVIEW matrix is automatically
pushed
			by the viewer with the tool-to-window affine returned by getMatrix() (multiplied by the
viewer's
			view affine). Use glGetDoublev() to retrieve the effective tool-to-window reference
change
			affine, and in case reimplement updateMatrix() to specify the affine returned by
getMatrix().
 \n\n
			The default implementation for updateMatrix() sets the tool reference to current
object's
			world one.

			  \par Tool Properties
			A tool's properties must be implemented by defining one or more TPropertyGroup
containers,
...
class DVAPI TTool
{
  ...
  public:
	...
	static void setApplication(TApplication *application) { m_application = application; }
	...
	static void
	updateToolsPropertiesTranslation(); //!< Updates translation of the bound properties of
  ...
  public:
	...
	*/
	...
	/*!
	This method is called before leftButtonDown() and can be used e.g. to create the image if
	needed.
   ...
   */
	...
	*/
	...
	void notifyImageChanged(
		const TFrameId &fid); //!< Notifies changes on the frame \p fid; used to update
							  //!  images on the level view.
	...
	TAffine
	getCurrentColumnParentMatrix() const; //!< Returns the current matrix transformation of the
  ...
  */
	...
	static const std::set<TFrameId> &getSelectedFrames() { return m_selectedFrames; }

  public:
	static std::vector<int> m_cellsData; //!< \deprecated  brutto brutto. fix quick & dirty del baco
										 //!#6213 (undo con animation sheet) spiegazioni in tool.cpp
...
class TTool::Viewer
{
  ...
  public:
	...
	virtual void
	invalidateAll() = 0; //!< Redraws the entire viewer, passing through Qt's event system
	virtual void GLInvalidateAll() = 0; //!< Redraws the entire viewer, bypassing Qt's event system
	virtual void GLInvalidateRect(
		const TRectD &rect) = 0; //!< Same as GLInvalidateAll(), for a specific clipping rect
	virtual void
	invalidateToolStatus() = 0; //!< Forces the viewer to update the perceived status of tools
	...
	virtual void posToColumnIndexes(const TPoint &p, std::vector<int> &indexes, double distance,
									bool includeInvisible = true) const = 0;
