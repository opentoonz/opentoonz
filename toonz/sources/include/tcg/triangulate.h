...
template <typename vertex_type> struct glu_vertex_traits {
...
template <typename TriMesh_type> struct ActionEvaluator {
...
template <typename TriMesh_type> struct DefaultEvaluator : public ActionEvaluator<TriMesh_type> {
	...
	*/
	...
	void actionSort(const TriMesh_type &mesh, int e,
...
/*!
  The algorithm accepts a tribe (that is, a container of containers, where each container class
  stores
  <I> pointer type objects <\I> to sub-containers) of polygons, and reads out a sequence of meshes
  with
  triangular faces.

  Each family of the tribe should contain a list of polygons representing a connected component
  to triangulate, where its first element is the external border, and the following ones are the
  internals.
...
void refineMesh(TriMesh_type &mesh, TriMeshStuff::ActionEvaluator<TriMesh_type> &eval,
