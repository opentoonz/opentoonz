...
*/
template <typename Scalar> inline int sign(Scalar val, Scalar tol = 0)
{
...
template <typename Scalar> Scalar trunc(Scalar val)
{
...
template <typename Scalar> Scalar growStrict(Scalar val)
{
...
template <typename Scalar> inline bool areNear(Scalar a, Scalar b, Scalar tolerance)
{
...
template <typename Scalar>
inline typename tcg::disable_if<tcg::is_floating_point<Scalar>::value, Scalar>::type mod(Scalar val,
																						 Scalar mod)
{
	Scalar m = val % mod;
	return (m >= 0) ? m : m + mod;
...
template <typename Scalar>
inline typename tcg::enable_if<tcg::is_floating_point<Scalar>::value, Scalar>::type mod(Scalar val,
																						Scalar mod)
{
...
template <typename Scalar> inline Scalar mod(Scalar val, Scalar a, Scalar b)
{
...
template <typename Scalar> inline Scalar modShift(Scalar val1, Scalar val2, Scalar m)
{
...
//! Returns the modular shift value with minimal abs:  <TT>mod(val2, a, b) = mod(val1 + shift, a,
//! b)</TT>
template <typename Scalar> inline Scalar modShift(Scalar val1, Scalar val2, Scalar a, Scalar b)
{
...
*/
template <typename Scalar>
inline typename tcg::disable_if<tcg::is_floating_point<Scalar>::value, Scalar>::type div(Scalar val,
																						 Scalar d)
{
...
*/
template <typename Scalar>
inline typename tcg::enable_if<tcg::is_floating_point<Scalar>::value, Scalar>::type div(Scalar val,
																						Scalar d)
{
...
*/
template <typename Scalar> inline Scalar div(Scalar val, Scalar a, Scalar b)
{
...
template <typename T, typename Scalar> inline T lerp(const T &v0, const T &v1, Scalar t)
{
	return (1 - t) * v0 + t * v1;
}
...
inline T bezier(const T &c0, const T &c1, const T &c2, Scalar t)
{
	...
	// return (c0 * one_t + c1 * t) * one_t + (c1 * one_t + c2 * t) * t;              // 6 Scalar-T
	// products
}
...
//! \deprecated
template <typename UScalar> inline UScalar GE_2Power(UScalar val)
{
	if (!val)
		return 0;
	--val;
