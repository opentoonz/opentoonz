...
template <typename V, typename E, typename F> class Mesh
{
  public:
  ...
  public:
	...
	void removeVertex(int v); //!< Removes the <TT>v</TT>-th vertex from the mesh.  \warning  Any
							  //!adjacent edge or face will be removed, too.  \param v  Index of the
							  //!vertex to be removed.
	void removeEdge(int e);   //!< Removes the <TT>e</TT>-th edge from the mesh.  \warning  Any
							  //!adjacent face will be removed, too.  \param e  Index of the edge to
							  //!be removed.
	void removeFace(int f);   //!< Removes the <TT>f</TT>-th face from the mesh.  \param f  Index of
	...
	{
		return m_vertices[v];
	} //!< Returns the <TT>v</TT>-th mesh vertex.   \param v  Index of the vertex to be returned.
	...
	{
		return m_edges[e];
	} //!< Returns the <TT>e</TT>-th mesh edge.   \param e  Index of the edge to be returned.
	  //!\return  See description.
	...
	{
		return m_faces[f];
	} //!< Returns the <TT>f</TT>-th mesh face.   \param f  Index of the face to be returned.
	  //!\return  See description.

	const V &edgeVertex(int e, int i) const { return vertex(edge(e).vertex(i)); }
	V &edgeVertex(int e, int i) //!  \param e  Host edge index.  \param i  Vertex index in e.
								//!  \return See description.
	{
	...
	F &edgeFace(
		int e,
		int i) //!  \param e  Host edge index.  \param i  Face index in e.  \return See description.
	{
		return face(edge(e).face(i));
	} //!< Returns the <TT>i</TT>-th face in the edge of index \p e.

	const V &otherEdgeVertex(int e, int v) const { return vertex(edge(e).otherVertex(v)); }
	V &otherEdgeVertex(int e, int v) //!  \param e  Host edge index.  \param v  Index of the
									 //!  adjacent vertex to \p e we're not interested in.  \return
									 //!  See description.
	{
		return vertex(edge(e).otherVertex(v));
	} //!< Retrieves the vertex adjacent to \p e whose index is \a not \p v.

	const F &otherEdgeFace(int e, int f) const { return face(edge(e).otherFace(f)); }
	F &otherEdgeFace(int e, int f) //!  \param e  Host edge index.  \param f  Index of the adjacent
								   //!  face to \p e we're not interested in.  \return See
								   //!  description.
	{
  ...
  */
	int edgeInciding(int v1, int v2, int n = 0)
		const; //!< \brief Returns the edge index of the <TT>n</TT>-th edge inciding
	//!  \p v1 and \p v2, or \p -1 if the required edge could not be found.   \param v1  First edge
	//!  endpoint.  \param v2  Second edge endpoint.  \param n  Index in the sequence of all edges
	//!  inciding \p v1 and \p v2.  \return  See description.
...
template <typename V, typename E, typename F> class TriMesh : public Mesh<V, E, F>
{
  protected:
  ...
  */
	...
	/*!
	\details    Specifically, this function removes <TT>edgeVertex(e, 1)</TT> and redirects its
	edges to
				<TT>edgeVertex(e, 0)</TT>. One edge per adjacent face (other than \p e) will be
	removed
				(which can be thought as 'merged' with the remaining one), and each adjacent face
	will be
				removed.
