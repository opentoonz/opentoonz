...
//! The class TFrameId describes a frame identified by an integer number of four figures and, in
//! case, by a character (necessary for added frames)
class DVAPI TFrameId
{
  ...
  public:
	...
	inline bool operator<(const TFrameId &f) const
	{
		return (m_frame < f.m_frame || (m_frame == f.m_frame && m_letter < f.m_letter));
	};
...
class DVAPI TFilePath
{
  ...
  public:
	/*! this static method allows correct reading of levels in the form pippo_0001.tif (represented
	 * always as pippo..tif) */
	...
	std::string getDots()
		const; // ritorna ""(no estensione), "."(estensione, no frame) o ".."(estensione e frame)

	/*!Returns the filename extension, including leading period (.).
		Returns "" if there is no filename extension.*/
	std::string
	getDottedType() const; // ritorna l'estensione con il PUNTO (se non c'e' estensione ritorna "")
	 ...
	 */
	std::string
	getLevelName() const; // es. TFilePath("/pippo/pluto.0001.gif").getLevelName() == "pluto..gif"
	std::wstring
	getLevelNameW() const; // es. TFilePath("/pippo/pluto.0001.gif").getLevelName() == "pluto..gif"
   ...
   type is a string that indicate the filename extension(ex:. bmp or .bmp)*/
	...
	TFilePath withFrame(const TFrameId &frame,
						TFrameId::FrameFormat format = TFrameId::FOUR_ZEROS) const;
	...
	inline TFilePath operator+(const std::string &s) const { return operator+(TFilePath(s)); };
	inline TFilePath &operator+=(const std::string &s) { return operator+=(TFilePath(s)); };
	...
	TFilePath operator-(const TFilePath &fp)
		const; // TFilePath("/a/b/c.txt")-TFilePath("/a/") == TFilePath("b/c.txt");
