...
public:
  ...
  virtual void activate(int col);
  virtual void deactivate(int col);
  virtual bool isActive(int col) const;
  virtual bool isEmpty() const { return m_columns.empty(); }
  virtual int count() const { return m_columns.size(); }
  virtual void saveData(TOStream &os);
  virtual void loadData(TIStream &is);
