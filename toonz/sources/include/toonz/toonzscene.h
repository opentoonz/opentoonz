...
class DVAPI ToonzScene
{
  public:
	...
	TXsheet *
	getTopXsheet() const; //!< Returns the \a topmost xsheet in the scene's xsheet hierarchy.
	...
	void
	loadNoResources(const TFilePath &path); //!< Loads a scene \a without loading its resources.
	...
	TFilePath getImportedLevelPath(
		const TFilePath path) const; //!< Builds the path to be used during a level import
	//!< operation.
	...
	bool convertLevelIfNeeded(
		TFilePath &levelPath); //!< Checks if the given levelPath is a file path of an
	...
	TXshLevel *loadLevel(
		...
		std::wstring levelName = L"", const std::vector<TFrameId> &fIds = std::vector<TFrameId>());
  ...
  */
	...
	32-bit raster.
  */
	void renderFrame(const TRaster32P &ras, int row, const TXsheet *xsh, const TRectD &placedRect,
  ...
  */
	TXshLevel *createNewLevel(int type, std::wstring levelName = L"",
							  const TDimension &dim = TDimension(), double dpi = 0,
