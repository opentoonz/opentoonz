...
struct DVAPI TargetColor {
  ...
  public:
	TargetColor(const TPixel32 &color, int index, int brightness, int contrast, double hRange,
				double threshold)
		: m_color(color), m_index(index), m_brightness(brightness), m_contrast(contrast),
		  m_hRange(hRange), m_threshold(threshold)
	{
	}
};
