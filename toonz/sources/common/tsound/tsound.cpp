...
TSoundTrack::TSoundTrack(TUINT32 sampleRate, int bitPerSample, int channelCount, int sampleSize,
						 TINT32 sampleCount, bool isSampleSigned)

	: TSmartObject(m_classCode), m_sampleRate(sampleRate), m_sampleSize(sampleSize),
	  m_bitPerSample(bitPerSample), m_sampleCount(sampleCount), m_channelCount(channelCount),
	  m_parent(0), m_bufferOwner(true)
{
...
TSoundTrack::TSoundTrack(TUINT32 sampleRate, int bitPerSample, int channelCount, int sampleSize,
						 TINT32 sampleCount, UCHAR *buffer, TSoundTrack *parent)

	: TSmartObject(m_classCode), m_sampleRate(sampleRate), m_sampleSize(sampleSize),
	  m_bitPerSample(bitPerSample), m_sampleCount(sampleCount), m_channelCount(channelCount),
	  m_parent(parent), m_buffer(buffer), m_bufferOwner(false)
{
	if (m_parent)
		m_parent->addRef();
}
...
TSoundTrackP TSoundTrack::create(TUINT32 sampleRate, int bitPerSample, int channelCount,
								 TINT32 sampleCount, bool signedSample)
{
	TSoundTrackP st;
...
TSoundTrackP TSoundTrack::create(TUINT32 sampleRate, int bitPerSample, int channelCount,
								 TINT32 sampleCount, void *buffer, bool signedSample)
{
	TSoundTrackP st;
	int type = bitPerSample + channelCount;
	switch (type) {
	...
	case TRK_M16:
		st = new TSoundTrackMono16(sampleRate, channelCount, sampleCount, (TMono16Sample *)buffer,
								   0);
		break;
	...
	case TRK_M24:
		st = new TSoundTrackMono24(sampleRate, channelCount, sampleCount, (TMono24Sample *)buffer,
								   0);
		break;
...
TSoundTrackP TSoundTrack::create(const TSoundTrackFormat &format, TINT32 sampleCount, void *buffer)
{
	return TSoundTrack::create((int)format.m_sampleRate, format.m_bitPerSample,
							   format.m_channelCount, sampleCount, buffer, format.m_signedSample);
}
...
{
	return TSoundTrack::create((int)format.m_sampleRate, format.m_bitPerSample,
							   format.m_channelCount, sampleCount, format.m_signedSample);
}
...
{
	return (m_sampleRate == rhs.m_sampleRate && m_bitPerSample == rhs.m_bitPerSample &&
			m_channelCount == rhs.m_channelCount && m_signedSample == rhs.m_signedSample);
}
...
{
	return TSoundTrackFormat(getSampleRate(), getBitPerSample(), getChannelCount(),
							 isSampleSigned());
}
...
void TSoundTrack::getMinMaxPressure(double t0, double t1, TSound::Channel chan, double &min,
									double &max) const
{
