...
void TRop::convert(const TRaster32P &rasOut, const TRasterCM32P &rasIn, const TPaletteP palette,
...
void do_convert(const TTile &dst, const TTile &src, const TPaletteP palette, bool transparencyCheck,
				bool applyFx)
{
...
void TRop::convert(const TRaster32P &rasOut, const TRasterCM32P &rasIn, TPaletteP palette,
...
void TRop::convert(const TTile &dst, const TTile &src, const TPaletteP plt, bool transparencyCheck,
				   bool applyFxs)
{
...
bool computePaletteFx(const std::vector<std::pair<TColorStyle *, int>> &fx, const TTile &tileOut,
					  const TTile &tileIn, const TPaletteP plt)
{
	int i;
	...
	for (i = 0; i < (int)fx.size(); i++) {
		...
		if (rfx->isPaintStyle())
			paintLayers[i] =
				putSinglePaintInRaster(rasIn, fx[i].second, fx[i].first->getMainColor());
		if (rfx->isInkStyle()) {
	...
	// ogni layer viene "effettato".; se il risultato e' non nullo, viene sovrapposto sul raster
	// d'uscita
	// prima vengono messi tutti i layer di paint, poi quelli di ink

	TRect rectOut =
		TRect(convert(tileOut.m_pos), tileOut.getRaster()->getSize()) - convert(tileIn.m_pos);

	for (i = 0; i < (int)fx.size(); i++)
		if (paintLayers[i]) {
			TRasterStyleFx::Params params(paintLayers[i], convert(tileIn.m_pos), rasIn,
										  fx[i].second, frame);
	...
	for (i = 0; i < (int)fx.size(); i++)
		if (inkLayers[i]) {
			...
			TRasterStyleFx::Params params(inkLayers[i], convert(tileIn.m_pos), rasIn, fx[i].second,
										  frame);
...
bool renderRas32(const TTile &tileOut, const TTile &tileIn, const TPaletteP palette)
{
	assert(TRect(convert(tileIn.m_pos), tileIn.getRaster()->getSize())
			   .contains(TRect(convert(tileOut.m_pos), tileOut.getRaster()->getSize())));
...
void doMergeCmapped(TRasterCM32P rasOut, const TRasterCM32P &rasUp, const TPaletteP &pltOut,
					bool onlyInks, int matchlinePrevalence, std::map<int, int> &usedInks)
{
	double val =
		matchlinePrevalence /
		100.0; // matchlinePrevalence ==0 always ink down; matchlinePrevalence == 100 always ink up;
	assert(rasOut && rasUp);
...
void doApplyMatchLines(TRasterCM32P rasOut, const TRasterCM32P &rasUp, int inkIndex,
					   int matchlinePrevalence)
{
	double val = matchlinePrevalence / 100.0; // matchlinePrevalence ==0->always ink down;
											  // matchlinePrevalence == 100 always ink up;
			...
			if (color != 0)
				for (i = 0; i < curColorIds.size() && curColorIds[i] <= color; i++)
					if (color == curColorIds[i]) {
						*pix = eraseInks ? TPixelCM32(0, pix->getPaint(), TPixelCM32::getMaxTone())
...
//--------------------------------
/*
void TRop::overaCmapped(TRasterCM32P rasOut, const TRasterCM32P& rasUp, const TPaletteP &pltOut, int
matchlinePrevalence, std::map<int,int>& usedColors)
{
...
void TRop::applyMatchLines(TRasterCM32P rasOut, const TRasterCM32P &rasUp, const TPaletteP &pltOut,
						   int inkIndex, int matchlinePrevalence, std::map<int, int> &usedInks)
{
...
void TRop::overlayCmapped(TRasterCM32P rasOut, const TRasterCM32P &rasUp, const TPaletteP &pltOut,
						  const TPaletteP &upPlt, std::map<int, int> &usedColors)
{
	...
	for (int y = 0; y < rasOut->getLy(); y++) {
		...
		for (int x = 0; x < rasOut->getLx(); x++, pixUp++, pixDown++) {
			if (isTransparent(pixUp)) // WARNING! cannot check transparent pixels with
									  // *pixup==TPixelCM32() since also 0x183000ff i.e., is a valid
									  // transparent value
				continue;
			int tone = pixUp->getTone();
			if (isTransparent(pixDown) || tone == 255 || tone == 0 ||
				pixUp->getPaint() != 0) // up e' punto interno, o esterno non antialiasato
			{
			...
			} else if (tone <= pixDown->getTone() ||
					   tone < 128) // up e' bordo esterno antialiasato piu' opaco di down
			{
...
_RASTER *TRop::convertRaster50to46(const TRasterP &inRas, const TPaletteP &inPalette)
{
	...
	memcpy(rout->cacheId, id.data(), sizeof(char) * rout->cacheIdLength);
	{
		...
		if (rcm)
			img = TToonzImageP(
				rcm, rcm->getBounds()); // saveBox is not considered in RASTER struct anyway
		else
...
{
	TImageP img(
		TImageCache::instance()->get(std::string(raster->cacheId, raster->cacheIdLength), true));
	TRasterP cacheRas;
...
inline TPixel32 getPix32(const TPixelCM32 &pixcm, const std::vector<TPixel32> &colors)
{
	int t = pixcm.getTone();
...
void TRop::zoomOutCm32Rgbm(const TRasterCM32P &rin, TRaster32P &rout, const TPalette &plt, int x1,
						   int y1, int x2, int y2, int newx, int newy, int absZoomLevel)
{
...
void TRop::makeIcon(TRaster32P &_rout, const TRasterCM32P &rin, const TPaletteP &palette,
					bool onBlackBg)
{
	int i, j;
	int lx = rin->getLx();
	...
	if (!areAlmostEqual(arIn, arOut, 1e-2)) // do not want a stretched icon! I extract a subraster
											// with same aspect ration of rin
	{
		int newlx, newly;
		if (arOut < arIn) {
			newlx = dim.lx;
