...
bool PolyStyle::operator==(const PolyStyle &p) const
{
	...
	case Centerline:
		...
		CASE Texture : return m_matrix == p.m_matrix &&
			m_texture.getPointer() == p.m_texture.getPointer();
		CASE LinearGradient : __OR RadialGradient : return m_color1 == p.m_color1 &&
			m_color2 == p.m_color2 && m_matrix == p.m_matrix && m_smooth == p.m_smooth;
	DEFAULT:
		assert(false);
		return false;
	}
