...
TToonzImage::TToonzImage(const TRasterCM32P &ras, const TRect &saveBox)
	: m_ras(ras), m_size(ras->getSize()), m_dpix(0), m_dpiy(0), m_subsampling(1), m_name(""),
	  m_savebox(saveBox)
	  //, m_hPos(0.0)
	  ,
	  m_offset(0, 0)
...
TToonzImage::TToonzImage(const TToonzImage &src)
	: m_ras(TRasterCM32P()), m_dpix(src.m_dpix), m_dpiy(src.m_dpiy),
	  m_subsampling(src.m_subsampling), m_name(src.m_name), m_savebox(src.m_savebox)
	  //, m_hPos(src.m_hPos)
	  ,
