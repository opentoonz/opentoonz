...
Now, the principal classes dealing with this job are:
 ...
 - The ResourceBuilder interface class, used by users to access the smart caching ensemble. This
class
...
class TFxCacheManager::Imp
{
  ...
  public:
	...
	void recursiveRectSubdivide(std::vector<ResourceDeclaration::TileData> &results, TRasterFx *fx,
...
void TFxCacheManager::declareResource(const std::string &alias, const TFxP &fx, const TRectD &rect,
									  double frame, const TRenderSettings &rs, bool subtileable)
{
	Imp::ResourceInstanceDataMap::iterator it;
	it = m_imp->m_resourcesData.insert(std::make_pair(alias, ResourceDeclaration())).first;
	it->second.m_rawData =
		&m_imp->m_rawData.insert(std::make_pair(&it->second, ResourceDeclaration::RawData()))
			 .first->second;
...
ResourceData TFxCacheManager::getResource(const std::string &alias, const TFxP &fx, double frame,
										  const TRenderSettings &rs)
{
	TCacheResourceP result, temp;

	// Seek the associated infos
	Imp::ResourceInstanceDataMap::iterator jt = m_imp->m_resourcesData.find(alias);
...
void TFxCacheManager::onRenderStatusStart(int renderStatus)
{
	...
	if (renderStatus == TRenderer::TESTRUN) {
		...
		DIAGNOSTICS_GLOSTRSET("testRenderStr",
		...
		DIAGNOSTICS_GLOSTRSET("compRenderStr",
...
void TFxCacheManager::onRenderStatusEnd(int renderStatus)
{
	if (renderStatus == TRenderer::FIRSTRUN) {
			...
			/*if(it->second.m_tiles.size() == 1 && it->second.m_simData->m_tiles.size() == 1)
	  {
		...
		DIAGNOSTICS_ADD(prefixErr + "Computing | Declarations survived after Test Run",
						resMap.size());
		if (resMap.size() > 0) {
...
//! Calculates at max 4 smaller subrects of passed one. Returns true or false whether the
//! subdivision was
//! successfully applied.
inline void TFxCacheManager::Imp::subdivideIntoSmallerTiles(const TRectD &rect,
...
void TFxCacheManager::Imp::recursiveRectSubdivide(
	std::vector<ResourceDeclaration::TileData> &results, TRasterFx *fx, const TRectD &rect,
	double frame, const TRenderSettings &info, int dropTol)
{
	...
	if ((memReq > info.m_maxTileSize && memReq < dropTol) ||
		...
		while (!subTileRects.empty()) {
			...
			recursiveRectSubdivide(results, fx, subTileRect, frame, info, memReq - (memReq >> 2));
...
ResourceBuilder::ResourceBuilder(const std::string &resourceName, const TFxP &fx, double frame,
								 const TRenderSettings &rs)
...
void ResourceBuilder::declareResource(const std::string &alias, const TFxP &fx, const TRectD &rect,
									  double frame, const TRenderSettings &rs, bool subtileable)
{
	TFxCacheManager::instance()->declareResource(alias, fx, rect, frame, rs, subtileable);
}
					 ...
					 std::vector<ResourceDeclaration::TileData *> &rectsToCalculate)
{
	...
	std::vector<ResourceDeclaration::TileData> &preparedRects = data.first->m_tiles;
...
void ResourceBuilder::simBuild(const TRectD &rect)
{
	...
	if (renderStatus == TRenderer::TESTRUN) {
		...
		DIAGNOSTICS_NUMBEREDSTRSET(
			...
			DIAGNOSTICS_STACKGET("parentResource") + " " + resName, ::traduce(rect), 4);
		...
		for (it = tiles.begin(); it != tiles.end(); ++it) {
			...
			if (!isEmpty(tileData.m_rect * rect)) {
				...
				if (tileData.m_refCount == 0) {
					...
					DIAGNOSTICS_PUSHAUTO("parentResource", QString::number(DIAGNOSTICS_GLOGET(
				...
				if (m_data.second) {
					...
					TRect tileRectI(tileData.m_rect.x0, tileData.m_rect.y0, tileData.m_rect.x1 - 1,
									tileData.m_rect.y1 - 1);
	...
	// will supposedly be NO MORE calculated, so the simCompute has to be launched on that tile,
	// too.
...
void ResourceBuilder::build(const TRectD &tileRect)
{
	...
	QString frameStr(DIAGNOSTICS_THRSTRGET("frameStr"));
	DIAGNOSTICS_NUMBEREDSTRSET(prefixComp + renderStr + frameStr, DIAGNOSTICS_THRSTRGET("stackVar"),
							   DIAGNOSTICS_STACKGET("parentResource") + " " + resName,
							   ::traduce(tileRect), 4);
	DIAGNOSTICS_PUSHAUTO("parentResource",
						 QString::number(DIAGNOSTICS_GLOGET(DIAGNOSTICS_THRSTRGET("stackVar"))),
	...
	if (!m_data.second) {
		...
		if (m_data.first)
			if (m_data.first->m_tilesCount > 0)
				DIAGNOSTICS_ADD(
					prefixErr + "Computing | No-resource build, active decl, tilesCount > 0", 1);
			else
				DIAGNOSTICS_ADD(
					prefixErr + "Computing | No-resource build, active decl, tilesCount <= 0", 1);
#endif
	...
	if (!ret)
		DIAGNOSTICS_STRSET(
			prefixErr + "Download falliti | " + DIAGNOSTICS_GLOSTRGET("compRenderStr") +
