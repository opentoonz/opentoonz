...
TRasterImage::TRasterImage(const TRasterP &ras)
	: m_mainRaster(ras), m_patchRaster(), m_iconRaster(), m_dpix(0), m_dpiy(0), m_name(""),
...
TRasterImage::TRasterImage(const TRasterImage &src)
	: m_mainRaster(src.m_mainRaster), m_patchRaster(src.m_patchRaster),
	  m_iconRaster(src.m_iconRaster), m_dpix(src.m_dpix), m_dpiy(src.m_dpiy), m_name(src.m_name),
	  m_savebox(src.m_savebox)
	  //, m_hPos(src.m_hPos)
	  ,
	  m_isOpaque(src.m_isOpaque), m_isScanBW(src.m_isScanBW), m_offset(src.m_offset),
...
void TRasterImage::makeIcon(const TRaster32P &dstRas)
{
	...
	TAffine aff = TScale(sc / dpix, sc / dpiy).place(srcRas->getCenterD(), dstRas->getCenterD());
