...
TString TSystemException::getMessage() const
{
	...
	case -1:
		// nothing
		msg = m_msg;
		CASE EEXIST : msg = L": Directory was not created because filename is the name of an "
							L"existing file, directory, or device";
		CASE ENOENT
			: msg =
				  L": Path was not found, or the named file does not exist or is a null pathname.";
		CASE ENOTEMPTY : msg = L": Given path is not a directory; directory is not empty; or "
							   L"directory is either current working directory or root directory";
		CASE EACCES : msg = L": Search permission is denied by a component of the path prefix, or "
							L"write permission on the file named by path is denied, or times is "
							L"NULL, and write access is denied";
		CASE EFAULT : msg = L": Times is not NULL and, or points outside the process's allocated "
							L"address space.";
		CASE EINTR : msg = L": A signal was caught during the utime system call.";
		CASE ENAMETOOLONG : msg = L": The length of the path argument exceeds {PATH_MAX}, or the "
								  L"length of a path component exceeds {NAME_MAX} while "
								  L"_POSIX_NO_TRUNC is in effect.";
		CASE ENOTDIR : msg = L": A component of the path prefix is not a directory.";
		CASE EPERM : msg = L": The calling process does not have the super-user privilege, the "
		...
		CASE EROFS : msg = L": The current file system level range does not envelop the level of "
						   L"the file named by path, and the calling process does not have the "
						   L"super-user privilege.";
		CASE ENOSYS : msg = L": When the named file cannot have its time reset.  The file is on a "
		...
		CASE EBADF : msg = L": The file descriptor determined by the DIR stream is no longer "
...
#ifndef MACOSX
		CASE EMULTIHOP : msg = L": Components of path require hopping to multiple remote machines "
							   L"and the file system does not allow it.";
		CASE ENOLINK : msg = L": Path points to a remote machine and the link to that machine is "
							 L"no longer active.";
#endif
#if defined(__sgi)
		CASE EDIRCORRUPTED : msg = L": The directory is corrupted on disk.";
#endif
		CASE EOVERFLOW : msg = L": One of the inode number values or offset values did not fit in "
							   L"32 bits, and the 64-bit interfaces were not used.";
#endif
	}
