...
private:
  ...
  bool hasChild(const TStroke *stroke,
                vector<shared_ptr<SubLayer>>::const_iterator &child) const;
};
...
SubLayers::SubLayers(ScreenMapper *mapper) : m_mapper(mapper) {
  connect(ImageManager::instance(), &ImageManager::updatedFrame, this,
          &SubLayers::onFrameUpdated);
}
...
shared_ptr<SubLayer> SubLayers::cell(const TXshColumn *column,
                                     int frameNumber) {
  if (!column) return empty();
...
shared_ptr<SubLayer> SubLayers::cell(const CellPosition &pos) {
  ...
  if (!cellId) return empty();

  map<TXshCell, shared_ptr<SubLayer>>::iterator it =
      m_cellSubLayers.find(*cellId);
  if (it != m_cellSubLayers.end()) return it->second;

  shared_ptr<SubLayer> newItem{build(*cellId)};
  m_cellSubLayers.insert(
      std::make_pair(*cellId, shared_ptr<SubLayer>(newItem)));
  return newItem;
}

optional<TXshCell> SubLayers::findCell(const CellPosition &pos) const {
  if (pos.layer() < 0) return boost::none;
  TXshColumn *column = m_mapper->xsheet()->getColumn(pos.layer());
  if (!column) return boost::none;
  TXshLevelColumn *levelColumn = column->getLevelColumn();
  if (!levelColumn) return boost::none;
...
void SubLayers::onFrameUpdated(const TXshCell &cellId) {
  map<TXshCell, shared_ptr<SubLayer>>::iterator it =
      m_cellSubLayers.find(cellId);
  if (it == m_cellSubLayers.end()) return;
...
SubLayer::SubLayer(SubLayers *subLayers, SubLayer *parent)
    : m_subLayers(subLayers), m_depth(0), m_folded(true) {
  if (parent) m_depth = parent->depth() + 1;
  connect(this, &SubLayer::foldToggled, m_subLayers->screenMapper(),
...
bool LayerSubLayer::hasChildren() const { return !m_column->isEmpty(); }

QString LayerSubLayer::name() const {
  int col = m_column->getIndex();
  TStageObjectId columnId =
      subLayers()->screenMapper()->viewer()->getObjectId(col);
...
QString CellSubLayer::name() const {
  QString levelName =
      QString::fromStdWString(m_cellId.getSimpleLevel()->getName());
...
void CellSubLayer::subscribeStrokeListChanged() {
  ...
  for (int i = 0; i < image->getStrokeCount(); i++)
    m_children.push_back(shared_ptr<StrokeSubLayer>(
        new StrokeSubLayer(subLayers(), this, image->getStroke(i))));

  connect(image.getPointer(), &TVectorImage::strokeListChanged, this,
          &CellSubLayer::onStrokeListChanged);
  m_subscribed = true;
}
