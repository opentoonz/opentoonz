...
class LicenseMsg : public TThread::Message
{
  ...
  public:
	...
	void onDeliver() { lastWarningError(m_msg); }

	TThread::Message *clone() const { return new LicenseMsg(*this); }
};

extern "C" void licenseObserverCB(char *msg)
{
	std::cout << __FUNCTION__ << " Begin message>" << std::endl;
	std::cout << msg << std::endl << "<End Message" << std::endl;
	LicenseMsg(msg).send();
}
...
/*
	...
	*check = 3;*/
	...
	// Controllo che la data della prima installazione contenuta nella sentinella sia minore della
	// data corrente.
