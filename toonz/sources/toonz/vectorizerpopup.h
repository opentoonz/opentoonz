...
class VectorizerPopup : public DVGui::Dialog
{
  ...
  public:
	...
	bool isLevelToConvert(
		TXshSimpleLevel *level); //!< Verifies if specified level supports vectorization.
  ...
  private slots:
	...
	void onTypeChange(
		int type); //!< Stores the active vectorization \p type name, and calls \p setType().
	...
	void onPartialDone(
		int partial,
		int total); //!< Partial progress \p partial of \p total was done for current frame.
	void onFinished();
...
class Vectorizer : public QThread
{
  ...
  public:
	...
	void setLevel(
		const TXshSimpleLevelP &level); //!< Stores current level and creates a new vector level.
  ...
  private:
	TXshSimpleLevelP
		m_level; //!< Input level to vectorize (only one level at a time is dealt here).
	...
	std::unique_ptr<OverwriteDialog> m_dialog; //!< Dialog to be shown for overwrite resolution.
  ...
  private:
	...
	TVectorImageP doVectorize(TImageP img, TPalette *palette, const VectorizerConfiguration &conf);
};
