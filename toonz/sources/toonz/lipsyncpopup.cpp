...
/*! \class LipSyncPopup
   ...
   apply lip sync text data to a image layer.
...
public:
	LipSyncUndo(int col, TXshSimpleLevel *sl, TXshLevelP cl, std::vector<TFrameId> activeFrameIds, QStringList textLines, int size,
		std::vector<TFrameId> previousFrameIds, std::vector<TXshLevelP> previousLevels, int startFrame);
...
LipSyncUndo::LipSyncUndo(int col, TXshSimpleLevel *sl, TXshLevelP cl, std::vector<TFrameId> activeFrameIds, QStringList textLines, int lastFrame,
	std::vector<TFrameId> previousFrameIds, std::vector<TXshLevelP> previousLevels, int startFrame)
	: m_col(col)
	, m_sl(sl)
	, m_cl(cl)
...
void LipSyncUndo::redo() const {
	...
	while (currentLine < m_textLines.size()) {
		...
		else endAt = m_textLines.at(currentLine + 2).toInt() - 1;
		if (endAt <= 0) break;
		if (endAt <= i) {
			currentLine += 2;
			continue;
		...
		while (i < endAt) {
			int currFrame = i + m_startFrame;
			TXshCell cell = xsh->getCell(currFrame, m_col);
			if (m_sl) cell.m_level = m_sl;
			else cell.m_level = m_cl;
...
LipSyncPopup::LipSyncPopup()
  ...
  setFixedWidth(860);
  setFixedHeight(400);
  m_applyButton = new QPushButton(tr("Apply Data"), this);
  m_aiLabel = new QLabel(tr("AI Drawing"));
  ...
  m_wqLabel = new QLabel(tr("WQ Drawing"));
  m_mbpLabel = new QLabel(tr("MBP Drawing"));
  m_fvLabel = new QLabel(tr("FV Drawing"));
  m_restLabel = new QLabel(tr("Rest Drawing"));
  m_otherLabel = new QLabel(tr("Other Drawing"));
  ...
  m_topLayout->setSpacing(0);
  {
	...
	phonemeLay->addWidget(m_imageLabels[j], 1, 0, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 1, 2, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 1, 4, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 1, 6, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 1, 8, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_navButtons[i], 2, 0, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 1, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 2, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 3, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 4, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 5, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 6, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 7, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 8, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 2, 9, Qt::AlignCenter); i++;
	...
	phonemeLay->addWidget(m_imageLabels[j], 4, 0, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 4, 2, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 4, 4, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 4, 6, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_imageLabels[j], 4, 8, 1, 2, Qt::AlignCenter); j++;
	phonemeLay->addWidget(m_navButtons[i], 5, 0, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 1, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 2, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 3, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 4, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 5, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 6, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 7, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 8, Qt::AlignCenter); i++;
	phonemeLay->addWidget(m_navButtons[i], 5, 9, Qt::AlignCenter); i++;
	phonemeLay->addWidget(new QLabel(tr("File: ")), 6, 0, 1, 1, Qt::AlignRight);
	phonemeLay->addWidget(m_file, 6, 1, 1, 7, Qt::AlignLeft);
	phonemeLay->addWidget(new QLabel(tr("Insert at Frame: ")), 6, 8, 1, 1, Qt::AlignRight);
	phonemeLay->addWidget(m_startAt, 6, 9, 1, 1, Qt::AlignLeft);
  ...
  { m_buttonLayout->addWidget(m_applyButton, Qt::AlignLeft); }

  //---- signal-slot connections
  QSignalMapper *signalMapper = new QSignalMapper(this);
  bool ret = true;
  
  ret = ret && connect(signalMapper, SIGNAL(mapped(int)), this, SLOT(imageNavClicked(int)));
  for (int i = 0; i < 20; i++) {
	  signalMapper->setMapping(m_navButtons[i], i);
	  ret = ret && connect(m_navButtons[i], SIGNAL(clicked()), signalMapper, SLOT(map()));
  }

  ret = ret &&
        connect(m_applyButton, SIGNAL(clicked()), this, SLOT(onApplyButton()));
  ret = ret && connect(m_file, SIGNAL(pathChanged()), this, SLOT(onPathChanged()));
  ret = ret && connect(m_startAt, SIGNAL(editingFinished()), this, SLOT(onStartValueChanged()));
...
void LipSyncPopup::showEvent(QShowEvent *) {
	...
	if (m_cl) DVGui::warning(tr("Thumbnails are not available for sub-Xsheets.\nPlease use the frame numbers for reference."));
	if (!m_sl && !m_cl) {
...
void LipSyncPopup::onApplyButton() {
	...
	LipSyncUndo *undo = new LipSyncUndo(m_col, m_sl, m_childLevel, m_activeFrameIds, m_textLines, lastFrame, previousFrameIds, previousLevels, startFrame);
...
void LipSyncPopup::imageNavClicked(int id) {
	...
	it = std::find(m_levelFrameIds.begin(), m_levelFrameIds.end(), currentFrameId);
	int frameIndex = std::distance(m_levelFrameIds.begin(), it);
	int newIndex;
	if (frameIndex == m_levelFrameIds.size() -1 && direction == 1) newIndex = 0;
	else if (frameIndex == 0 && direction == -1) newIndex = m_levelFrameIds.size() - 1;
	else newIndex = frameIndex + direction;
	m_activeFrameIds[frameNumber] = m_levelFrameIds.at(newIndex);
	TXshCell newCell = TApp::instance()->getCurrentScene()->getScene()->getXsheet()->getCell(30, m_col);
...
void LipSyncPopup::paintEvent(QPaintEvent *) {
	if (m_sl || m_cl) {
		int i = 0;
		while (i < 10) {
			//TXshCell cell = TApp::instance()->getCurrentScene()->getScene()->getXsheet()->getCell(i, m_col);
			QPixmap pm;
			if (m_sl) pm = IconGenerator::instance()->getIcon(m_sl, m_activeFrameIds[i]);
			...
			if (m_cl) {
				...
				p.drawText(r, tr("SubXSheet Frame ") + QString::number(frameIndex + 1), QTextOption(Qt::AlignCenter));
...
void LipSyncPopup::onPathChanged() {
	...
	if (!file.open(QIODevice::ReadOnly)) {
		DVGui::warning(tr("Unable to open the file: /n") + file.errorString());
	}
