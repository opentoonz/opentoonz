...
TRaster32P loadBravo(const TDimension &res)
{
	...
	if (Res != res) {
		...
		if (outLx == 160 && outLy == 60)
			...
			TRop::resample(rasOut, ras, TScale(outLx / 160.0, outLy / 60.0)
											.place(ras->getCenterD(), rasOut->getCenterD()));
		}
		Res = res;
	}
...
void RenderCommand::flashRender()
{
	...
	for (i = 0; i < m_numFrames; ++i, m_r += m_stepd) {
		...
		flash.multMatrix(TTranslation((cameraSize.lx - lx) / 2 - (dx > 0 ? dx : 0),
									  -(cameraSize.ly - ly) / 2 + (dy > 0 ? dy : 0)));
		flash.draw(ri, 0);
		flash.popMatrix();
#endif
