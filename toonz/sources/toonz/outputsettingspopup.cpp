...
QFrame *OutputSettingsPopup::createMoreSettingsBox() {
  ...
  // Put Layout Image
  m_putLayoutImage = new DVGui::CheckBox(tr("Put Layout Image"), this);
  // Gamma
...
void OutputSettingsPopup::updateField() {
  ...
  m_putLayoutImage->setChecked(prop->isPutLayoutImage());
}
...
void OutputSettingsPopup::onPutLayoutImageChecked(int state) {
  if (state == getProperties()->isPutLayoutImage()) return;
  getProperties()->setPutLayoutImage(state);
}
