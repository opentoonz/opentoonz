...
bool TabletSupportWinInk::init()
{
	return api.init();
}

bool TabletSupportWinInk::nativeEventFilter(const QByteArray &eventType, void *message, long *result)
{
	if (!result) {
...
namespace {
	...
	QTabletEvent makePositionalTabletEvent(const QWidget *targetWidget, const QEvent::Type eventType, const POINTER_PEN_INFO &penInfo, const PointerDeviceItem &deviceItem, const PenPointerItem &penPointerItem)
	{
		...
		if (eventType == QEvent::TabletPress) {
			...
			if (penInfo.pointerInfo.ButtonChangeType == POINTER_CHANGE_SECONDBUTTON_UP) {
				...
				if(!(penInfo.pointerInfo.ButtonChangeType == POINTER_CHANGE_FIRSTBUTTON_UP)) {
					qWarning() << "WM_POINTER* sent unknown ButtonChangeType" << penInfo.pointerInfo.ButtonChangeType;
				}
				mouseButton = Qt::LeftButton;
			}
	...
	{
		if(!(eventType == QEvent::TabletEnterProximity || eventType == QEvent::TabletLeaveProximity)) {
			return false;
		}
	...
	{
		if(!(eventType == QEvent::TabletMove || eventType == QEvent::TabletPress || eventType == QEvent::TabletRelease)) {
			return false;
		}
