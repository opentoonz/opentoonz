...
//		layerAxisRange = NumberRange(layerAxis + 1, layerAxis + adjY);
		layerAxisRange = NumberRange(layerAxis + 1, m_viewer->columnToLayerAxis(col - 1) - 1);
	}
...
void CellArea::drawCurrentTimeIndicator(QPainter &p, const QPoint &xy, int adjLength) {
  int frameAdj = m_viewer->getFrameZoomAdjustment();
  QRect cell =
...
void CellArea::drawSubLayers(QPainter &p, const CellPosition &pos) {
  ...
  if (TApp::instance()->getCurrentFrame()->isEditingScene() &&
      ...
      row == m_viewer->getCurrentRow() &&
      Preferences::instance()->isCurrentTimelineIndicatorEnabled())
    drawCurrentTimeIndicator(p, xy, (layerAxis.length() - layerOffset + 1));
}
