...
*/
FlipBook::FlipBook(QWidget *parent, QString viewerTitle, UINT flipConsoleButtonMask, UCHAR flags,
	  ...
	  m_flags(flags)
{
	...
	m_imageViewer =
		new ImageViewer(fsWidget, this, flipConsoleButtonMask == FlipConsole::cFullConsole);
	...
	mainLayout->setSpacing(0);
	{
		...
		m_flipConsole =
			new FlipConsole(mainLayout, flipConsoleButtonMask, true, 0,
	...
	bool ret = connect(m_flipConsole, SIGNAL(buttonPressed(FlipConsole::EGadget)), this,
	...
	ret = ret &&
		  connect(parentWidget(), SIGNAL(closeButtonPressed()), this, SLOT(onCloseButtonPressed()));
...
void FlipBook::addFreezeButtonToTitleBar()
{
	...
	if (panel) {
		...
		m_freezeButton =
			new TPanelTitleBarButton(titleBar, ":Resources/freeze.png",
...
LoadImagesPopup::LoadImagesPopup(FlipBook *flip)
	: FileBrowserPopup(tr("Load Images"), Options(WITH_APPLY_BUTTON), tr("Append"), new QFrame(0)),
	  m_flip(flip), m_minFrame(0), m_maxFrame(1000000), m_step(1), m_shrink(1)
{
...
bool LoadImagesPopup::doLoad(bool append)
{
	...
	::viewFile(*m_selectedPaths.begin(), m_from, m_to, m_step, m_shrink, 0, m_flip, append);
...
bool FlipBook::doSaveImages(TFilePath fp)
{
	...
	if (!formats.contains(QString::fromStdString(ext))) {
		DVGui::warning(
			tr("It is not possible to save because the selected file format is not supported."));
		return false;
	}
	...
	if (TFileType::getInfo(fp) == TFileType::RASTER_IMAGE || ext == "pct" || ext == "pic" ||
		ext == "pict") // pct e' un formato"livello" (ha i settings di quicktime) ma fatto di
					   // diversi frames
...
void FlipBook::saveImage()
{
	...
	QString str = tr("Saved %1 frames out of %2 in %3")
					  .arg(toString(savedFrames).c_str())
					  .arg(toString(m_framesCount).c_str())
					  .arg(toString(m_lw->getFilePath().getWideString()).c_str());
...
void FlipBook::onButtonPressed(FlipConsole::EGadget button)
{
	...
	case FlipConsole::eSaveImg: {
		...
		if (!img) {
			...
			DVGui::warning(
				tr("It is not possible to take or compare snapshots for Toonz vector levels."));
			return;
		}
...
bool FlipBook::isSavable() const
{
	...
	for (int i = 0; i < m_levels.size(); i++)
		if (m_levels[i].m_fp != TFilePath() &&
			(m_levels[i].m_fp.getType() == "tlv" || m_levels[i].m_fp.getType() == "pli"))
			return false;
...
*/
void FlipBook::setLevel(const TFilePath &fp, TPalette *palette, int from, int to, int step,
						int shrink, TSoundTrack *snd, bool append, bool isToonzOutput)
{
	try {
		if (!append) {
			clearCache();
		...
		if (TSystem::doesExistFileOrLevel(fp)) // is a  viewfile
		{
			...
			// For the color model, get the reference fids from palette and delete unneeded from the
			// table
			if (m_imageViewer->isColorModel() && palette) {
				std::vector<TFrameId> fids = palette->getRefLevelFids();

				// when loading a single-frame, standard raster image into the ColorModel, skip
				// here.
			...
			// in order to avoid that the current frame unexpectedly moves to 1 on the Color Model
			// once editing the style
			int current = -1;
			...
			if (from == -1 && to == -1) {
				...
				//  If the level read did not support random access, *AND* the level to show was
				//  just rendered,
				//  we have to assume that no level update happened, and the from-to-step infos are
				//  lost.
			...
			// when using the flip module, this signal is to show the loaded level names in
			...
			// An old archived bug says that simulatenous open for read of the same tlv are not
			// allowed...
...
void FlipBook::setLevel(TXshSimpleLevel *xl)
{
	try {
		...
		if (p)
			m_title = m_title + "  ::  " + QString::number(p->getImageRes().lx) + "x" +
					  QString::number(p->getImageRes().ly) + " Pixels";
...
void FlipBook::setLevel(TFx *previewedFx, TXsheet *xsh, TLevel *level, TPalette *palette, int from,
						int to, int step, int currentFrame, TSoundTrack *snd)
{
	m_xl = 0;
...
TRectD FlipBook::getPreviewedImageGeometry() const
{
	...
	TRectD viewerGeomD(viewerGeom.left(), viewerGeom.top(), viewerGeom.right() + 1,
					   viewerGeom.bottom() + 1);
	...
	TAffine viewToCam(m_imageViewer->getViewAff().inv() * TTranslation(-viewerCenter));
		...
		try {
			...
			if (e.getType() == TSoundDeviceException::UnsupportedFormat) {
				try {
					...
					m_player->play(TSop::convert(m_snd, fmt), firstSample, lastSample, false,
								   false);
...
TImageP FlipBook::getCurrentImage(int frame)
{
	...
	} else if (!m_levels.empty()) // is a viewfile or a previewFx
	{
		...
		int i = 0;
		// Search all subsequent levels on the flipbook and retrieve the one containing the required
		// frame
	...
	if (fp != TFilePath() && !m_isPreviewFx) {
		...
		if (img) {
			...
			if (ri && showSub && m_loadbox != TRect() &&
				ri->getRaster()->getLx() == oriLx) // questo serve perche' per avi e mov la
												   // setRegion e' completamente ignorata...
	...
	if (!m_levels.empty()) // is a viewfile
		for (i = 0; i < m_levels.size(); i++)
			for (it = m_levels[i].m_level->begin(); it != m_levels[i].m_level->end(); ++it)
				TImageCache::instance()->remove(m_levelNames[i].toStdString() +
												toString(it->first.getNumber()) +
												((m_isPreviewFx) ? "" : toString(this)));
	else {
		int from, to, step;
		m_flipConsole->getFrameRange(from, to, step);
...
void FlipBook::adaptGeometry(const TRect &interestingImgRect, const TRect &imgRect)
{
	...
	adaptWidGeometry(TRect(tceil(interestGeomD.x0), tceil(interestGeomD.y0),
						   tfloor(interestGeomD.x1) - 1, tfloor(interestGeomD.y1) - 1),
					 TRect(tceil(imageGeomD.x0), tceil(imageGeomD.y0), tfloor(imageGeomD.x1) - 1,
						   tfloor(imageGeomD.y1) - 1),
					 true);
}
...
void FlipBook::adaptGeometryForFullPreview(const TRect &imgRect)
{
	...
	while (1) {
		...
		if (imageGeom.getLx() <= screenGeom.width() && imageGeom.getLy() <= screenGeom.height()) {
...
void FlipBook::adaptWidGeometry(const TRect &interestWidGeom, const TRect &imgWidGeom,
								bool keepPosition)
{
	TPanel *panel = static_cast<TPanel *>(parentWidget());
	if (!panel->isFloating())
	...
	QSize minAddition(tceil(tmax(0, flipMinimumSize.width() - interestGeom.width()) * 0.5),
					  tceil(tmax(0, flipMinimumSize.height() - interestGeom.height()) * 0.5));
	interestGeom.adjust(-minAddition.width(), -minAddition.height(), minAddition.width(),
						minAddition.height());
	...
	QPointF newViewerGeomCenter((newViewerGeom.left() + newViewerGeom.right() + 1) * 0.5,
  ...
  QPointF newViewerGeomCenter(
	...
	// NOTE: If delta == (0,0) the image is at center. Typically happens when imageGeom doesn't
	// intersect
	// the screen geometry.
...
void FlipBook::onDoubleClick(QMouseEvent *me)
{
	...
	// with respect to the widget - the problem is, we cannot take the closest integer rect
	// ENCLOSING ours,
...
*/
void FlipBook::loadAndCacheAllTlvImages(Level level, int fromFrame, int toFrame)
{
...
//! to specific frames).
void viewFile(const TFilePath &path, int from, int to, int step, int shrink, TSoundTrack *snd,
			  FlipBook *flipbook, bool append, bool isToonzOutput)
{
	...
	if ((path.getType() == "mov" || path.getType() == "avi" || path.getType() == "3gp") &&
		path.isLevelName()) {
