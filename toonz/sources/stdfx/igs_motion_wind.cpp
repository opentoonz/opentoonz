...
{
template <class T>
const T *ref_pixel_(const T *ref, const int hh, const int ww, const int cc, int xx, int yy)
{
	while (xx < 0) {
		xx += ww;
	}
...
template <class T> void pixel_pop_(const T *in, const double div_val, const int cc, double *out)
{
	/* 正規化し取り出し */
...
template <class T> void pixel_push_(const double *in, const double mul_val, const int cc, T *out)
{
	/* 値が変化したら格納する */
	for (int zz = 0; zz < cc; ++zz) {
...
void change_template_vert_(T *in, const int ww, const int hh, const int cc, const R *ref,
						   const int rw, const int rh, const int rc
						   //, const int rz
						   ,
	...
	for (int xx = 0; xx < ww; ++xx) {
		...
		if (pix_rend.change(xx <= 0, rz, rp_p, cc, pixel_in)) {
			...
			if (0 != ref && 0 <= ref_mode && ref_mode <= 4) {
				ref_val = igs::color::ref_value(ref_pixel_(ref, rh, rw, rc, xx, yy + refprev), rc,
												ref_max_val, ref_mode);
			}
...
template <class T, class R>
void change_template_hori_(T *in, const int ww, const int hh, const int cc, const R *ref,
						   const int rw, const int rh, const int rc
						   //, const int rz
						   ,
						   const int ref_mode /* 0=R,1=G,2=B,3=A,4=Luminance,5=Nothing */
...
void igs::motion_wind::change(unsigned char *in
							  ...
							  )
{
	...
	igs::motion_wind::pixel pix_rend(
		blow_dark_sw, blow_alpha_sw, length_random_seed, length_min, length_max, length_bias,
		length_ref_sw, force_random_seed, force_min, force_max, force_bias, force_ref_sw,
		density_random_seed, density_min, density_max, density_bias, density_ref_sw);
	//------------
	...
	if (std::numeric_limits<unsigned char>::digits == bi) {
		if (std::numeric_limits<unsigned short>::digits == refbi) {
			if ((0 == direction) || (2 == direction)) {
				change_template_hori_(in, ww, hh, cc, reinterpret_cast<const unsigned short *>(ref),
									  rw, rh, rc
									  //,rz
									  ,
									  ref_mode, pix_rend, (0 == direction) ? false : true);
			} else if ((1 == direction) || (3 == direction)) {
				change_template_vert_(in, ww, hh, cc, reinterpret_cast<const unsigned short *>(ref),
									  rw, rh, rc
									  //,rz
									  ,
		...
		if (std::numeric_limits<unsigned short>::digits == refbi) {
			if ((0 == direction) || (2 == direction)) {
				change_template_hori_(reinterpret_cast<unsigned short *>(in), ww, hh, cc,
									  reinterpret_cast<const unsigned short *>(ref), rw, rh, rc
									  //,rz
									  ,
									  ref_mode, pix_rend, (0 == direction) ? false : true);
			} else if ((1 == direction) || (3 == direction)) {
				change_template_vert_(reinterpret_cast<unsigned short *>(in), ww, hh, cc,
									  reinterpret_cast<const unsigned short *>(ref), rw, rh, rc
									  //,rz
									  ,
									  ref_mode, pix_rend, (1 == direction) ? false : true);
			...
			if ((0 == direction) || (2 == direction)) {
				change_template_hori_(reinterpret_cast<unsigned short *>(in), ww, hh, cc, ref, rw,
									  rh, rc
									  //,rz
									  ,
									  ref_mode, pix_rend, (0 == direction) ? false : true);
			} else if ((1 == direction) || (3 == direction)) {
				change_template_vert_(reinterpret_cast<unsigned short *>(in), ww, hh, cc, ref, rw,
									  rh, rc
									  //,rz
									  ,
									  ref_mode, pix_rend, (1 == direction) ? false : true);
