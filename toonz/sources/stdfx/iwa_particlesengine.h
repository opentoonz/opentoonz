...
struct ParticleOrigin {
	...
	ParticleOrigin(float x, float y, float _potential, bool _isUpward, unsigned char _level,
				   unsigned char _initSourceFrame, short int pix_x, short int pix_y)
	{
		pos[0] = x;
		pos[1] = y;
...
class Iwa_Particles_Engine
{
  ...
  public:
	...
	void fill_range_struct(struct particles_values &values, struct particles_ranges &ranges);

	void fill_value_struct(struct particles_values &value, double frame);

	void roll_particles(TTile *tile, std::map<int, TTile *> porttiles, const TRenderSettings &ri,
						std::list<Iwa_Particle> &myParticles, struct particles_values &values,
						float cx, float cy, int frame, int curr_frame, int level_n,
						bool *random_level, float dpi, std::vector<int> lastframe,
						int &totalparticles, QList<ParticleOrigin> &particleOrigin, int genPartNum);

	void normalize_values(struct particles_values &values, const TRenderSettings &ri);

	void render_particles(TFlash *flash, TTile *tile, std::vector<TRasterFxPort *> part_ports,
	...
	void do_render(TFlash *flash, Iwa_Particle *part, TTile *tile,
				   ...
				   const TRenderSettings &ri, TDimension &p_size, TPointD &p_offset, int lastframe,
				   std::vector<TLevelP> partLevel, struct particles_values &values,
				   float opacity_range, int curr_frame,
				   std::map<std::pair<int, int>, float> &partScales, TTile *baseImgTile);

	bool port_is_used(int i, struct particles_values &values);

	void fill_single_region(std::vector<TPointD> &myregions, TTile *ctrl1, int thres,
							bool do_source_gradation, QList<QList<int>> &myHistogram,
							QList<ParticleOrigin> &particleOrigins);

	void fill_subregions(int cont_index, std::vector<std::vector<TPointD>> &myregions, TTile *ctrl1,
						 int thres);

	void normalize_array(std::vector<std::vector<TPointD>> &myregions, TPointD pos, int lx, int ly,
	...
	void initParticleOrigins(TRectD &outTileBBox, QList<ParticleOrigin> &particleOrigins,
	...
	unsigned char getInitSourceFrame(const particles_values &values, int first, int last);
