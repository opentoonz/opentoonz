...
class ino_blend_linear_burn : public TBlendForeBackRasterFx
{
  ...
  public:
	ino_blend_linear_burn() : m_opacity(1.0 * ino::param_range()), m_clipping_mask(true)
	{
...
void ino_blend_linear_burn::computeUpAndDown(TTile &tile, double frame, const TRenderSettings &rs,
...
void ino_blend_linear_burn::dryComputeUpAndDown(TRectD &rect, double frame,
												const TRenderSettings &rs, bool upComputesWholeTile)
{
	const bool up_is =
		(this->m_up.isConnected() && this->m_up.getFx()->getTimeRegion().contains(frame));
	const bool down_is =
...
void ino_blend_linear_burn::doCompute(TTile &tile, double frame, const TRenderSettings &rs)
{
	/* ------ 画像生成 ---------------------------------------- */
