...
{
  public:
	noise_reference_(const int ww, const int hh, const double hue_range, const double sat_range,
...
void pixel_rgb_(const double red_in, const double gre_in, const double blu_in, const double alp_in,
				...
				control_term_within_limits_ &sat_term, control_term_within_limits_ &val_term,
	...
	if (igs::image::rgba::siz == channels) {
		...
		Alpha値がゼロでもRGB値は存在する(してもよい) */
				...
				if (((0.0 != hue_range) || (0.0 != val_term.noise_range()) ||
					...
					pixel_rgb_(rr1, gg1, bb1, aa1, noise.hue_value(xx, yy), noise.sat_value(xx, yy),
							   noise.val_value(xx, yy), sat_term, val_term, rr2, gg2, bb2);
					if (refv != 1.0) {
			...
			for (int yy = 0; yy < height; ++yy) {
				for (int xx = 0; xx < width; ++xx, image_array += channels) {
					...
					double rr1 = static_cast<double>(image_array[red]) / div_val,
						   ...
						   bb1 = static_cast<double>(image_array[blu]) / div_val;
					double rr2 = 0, gg2 = 0, bb2 = 0;
					pixel_rgb_(rr1, gg1, bb1, 1.0, noise.hue_value(xx, yy), noise.sat_value(xx, yy),
							   noise.val_value(xx, yy), sat_term, val_term, rr2, gg2, bb2);
					if (refv != 1.0) {
						rr2 = (rr2 - rr1) * refv + rr1;
						gg2 = (gg2 - gg1) * refv + gg1;
...
void igs::hsv_noise::change(unsigned char *image_array
							...
							const bool add_blend_sw)
{
	...
	/* ノイズ参照画像を作成する */
	noise_reference_ noise(width, height, hue_range, sat_range, val_range, alp_range, random_seed,
	...
	control_term_within_limits_ sat_term(sat_effective, sat_effective, sat_center, sat_type,
										 sat_range);
	control_term_within_limits_ val_term(val_effective, val_effective, val_center, val_type,
										 val_range);
	...
	if ((std::numeric_limits<unsigned char>::digits == bits) &&
		...
		change_template_(reinterpret_cast<unsigned short *>(image_array), width, height, channels,
						 ref, ref_mode, noise, hue_range, sat_term, val_term, alp_term,
						 add_blend_sw);
		noise.clear(); /* ノイズ画像メモリ解放 */
	} else if ((std::numeric_limits<unsigned short>::digits == bits) &&
