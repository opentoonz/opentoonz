...
void Particle::create_Animation(const particles_values &values, int first, int last)
{
	...
	case ParticlesFx::ANIM_CYCLE:
		...
		CASE ParticlesFx::ANIM_SR_CYCLE : frame =
											  (int)(first + (random.getFloat()) * (last - first));
...
void Particle::update_Animation(const particles_values &values, int first, int last, int keep)
{
	...
	case ParticlesFx::ANIM_RANDOM:
		frame = (int)(first + random.getFloat() * (last - first));
		CASE ParticlesFx::ANIM_R_CYCLE : __OR ParticlesFx::ANIM_CYCLE
								   ...
								   double &imagereference, int type)
{
	...
	case ParticlesFx::GRAY_REF:
		...
		CASE ParticlesFx::H_REF : if (raster32 && tmp.x >= 0 && tmp.x < raster32->getLx() &&
