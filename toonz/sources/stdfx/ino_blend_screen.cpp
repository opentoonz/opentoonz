...
class ino_blend_screen : public TBlendForeBackRasterFx
{
  ...
  public:
	ino_blend_screen() : m_opacity(1.0 * ino::param_range()), m_clipping_mask(true)
	{
...
void ino_blend_screen::computeUpAndDown(TTile &tile, double frame, const TRenderSettings &rs,
...
void ino_blend_screen::dryComputeUpAndDown(TRectD &rect, double frame, const TRenderSettings &rs,
										   bool upComputesWholeTile)
{
	const bool up_is =
		(this->m_up.isConnected() && this->m_up.getFx()->getTimeRegion().contains(frame));
...
void ino_blend_screen::doCompute(TTile &tile, double frame, const TRenderSettings &rs)
{
	/* ------ 画像生成 ---------------------------------------- */
