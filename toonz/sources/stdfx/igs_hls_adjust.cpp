...
void pixel_rgba_(const double red_in, const double gre_in, const double blu_in, double &red_out,
				 double &gre_out, double &blu_out, const double hue_pivot // 0.0  ...0...360...
				 ,
					  ...
					  const bool add_blend_sw)
{
	...
	if (igs::image::rgba::siz == channels) {
		...
		for (int ii = 0; ii < pixsize; ++ii, image_array += channels) {
			...
			double rr2, gg2, bb2;
			pixel_rgba_(rr1, gg1, bb1, rr2, gg2, bb2, hue_pivot, hue_scale, hue_shift, lig_pivot,
						lig_scale, lig_shift, sat_pivot, sat_scale, sat_shift);
...
void igs::hls_adjust::change(unsigned char *image_array, const int height, const int width,
							 ...
							 const bool add_blend_sw)
{
	if ((1.0 == hue_scale) && (0.0 == hue_shift) && (1.0 == lig_scale) && (0.0 == lig_shift) &&
