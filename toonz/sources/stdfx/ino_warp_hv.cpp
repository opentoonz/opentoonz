...
class ino_warp_hv : public TStandardRasterFx
{
  ...
  public:
	ino_warp_hv()
		: m_h_maxlen(0.0 * ino::param_range()), m_v_maxlen(0.0 * ino::param_range()),
		  m_h_ref_mode(new TIntEnumParam(2, "Red")), m_v_ref_mode(new TIntEnumParam(2, "Red")),
		  m_alpha_rendering(true), m_anti_aliasing(true)
	{
		...
		this->m_h_maxlen->setValueRange(0.0 * ino::param_range(), 100.0 * ino::param_range());
		this->m_v_maxlen->setValueRange(0.0 * ino::param_range(), 100.0 * ino::param_range());
	...
	void get_render_real_hv(const double frame, const TAffine affine, double &h_maxlen,
							double &v_maxlen)
	{
		/*--- ベクトルにする(プラス値) --- */
		TPointD rend_vect;
...
void fx_(TRasterP in_ras // with margin
		 ...
		 ,
		 const double h_maxlen, const double v_maxlen, const int h_ref_mode, const int v_ref_mode,
...
//------------------------------------------------------------
void ino_warp_hv::doCompute(TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	/*------ 接続していなければ処理しない ----------------------*/
	...
	if (log_sw) {
		...
		os << "params"
		   << "  h_maxlen " << h_maxlen << "  v_maxlen " << v_maxlen << "  margin " << margin
		   << "  alp_rend_sw " << alp_rend_sw << "  anti_ali_sw " << anti_ali_sw
	...
	try {
		...
		fx_(enlarge_tile.getRaster() // in with margin
			...
			,
			h_maxlen, v_maxlen, this->m_h_ref_mode->getValue(), this->m_v_ref_mode->getValue(),
			alp_rend_sw, anti_ali_sw);
		if (vert_cn_is) {
