...
class ino_spin_blur : public TStandardRasterFx
{
  ...
  public:
	ino_spin_blur()
		: m_center(TPointD(0.0, 0.0)), m_blur(1), m_radius(0.0),
		  m_type(new TIntEnumParam(0, "Accelerator")), m_alpha_rendering(true), m_anti_alias(false),
							  ...
							  TPointD center)
	{
		...
		return igs::rotate_blur::reference_margin(
			...
			center.y, this->m_blur->getValue(frame), this->m_radius->getValue(frame) * scale,
...
void fx_(const TRasterP in_ras // with margin
		 ...
		 ,
		 const double xp, const double yp, const int type, const double blur, const double radius,
		 const bool alpha_rendering_sw, const bool anti_alias_sw)
{
	...
	igs::rotate_blur::convert(in_ras->getRawData() // BGRA
							  ,
...
//------------------------------------------------------------
void ino_spin_blur::doCompute(TTile &tile, double frame, const TRenderSettings &ri)
{
	/*------ 接続していなければ処理しない ----------------------*/
	if (!this->m_input.isConnected()) {
	...
	/*------ 入力画像のエリアの拡大 ----------------------------*/
	int margin = this->get_render_int_margin(frame, bBox, ri.m_affine, render_center);
	if (0 < margin) {
		/* 拡大のしすぎを防ぐテキトーな制限 */
		if (4096 < margin) {
			margin = 4096;
	...
	if (log_sw) {
		...
		os << "params"
		   ...
		   << radius << "  type " << type << "  reference " << ref_mode << "  alpha_rendering "
		   << alpha_rend_sw << "  anti_alias " << anti_alias_sw << "  rend_cx " << render_center.x
		   << "  rend_cy " << render_center.y << "  tx " << tile.m_pos.x << "  ty " << tile.m_pos.y
		   << "  tw " << tile.getRaster()->getLx() << "  th " << tile.getRaster()->getLy()
		   << "  tb " << ino::pixel_bits(tile.getRaster()) << "  margin " << margin << "  bx0 "
		   << bBox.x0 << "  by0 " << bBox.y0 << "  bx1 " << bBox.x1 << "  by1 " << bBox.y1
		   << "  ix " << in_enlarge_tile.m_pos.x << "  iy " << in_enlarge_tile.m_pos.y << "  iw "
		   << in_enlarge_tile.getRaster()->getLx() << "  ih "
		   << in_enlarge_tile.getRaster()->getLy();
		if (reference_sw) {
			os << "  rx " << reference_tile.m_pos.x << "  ry " << reference_tile.m_pos.y << "  rw "
			   << reference_tile.getRaster()->getLx() << "  rh "
			   << reference_tile.getRaster()->getLy();
		}
		os << "  frame " << frame << "  ri_aff_det " << ri.m_affine.det() << "  shrink_x "
		   << ri.m_shrinkX << "  shrink_y " << ri.m_shrinkY;
	}
	/* ------ fx処理 ------------------------------------------ */
	try {
		tile.getRaster()->lock();
