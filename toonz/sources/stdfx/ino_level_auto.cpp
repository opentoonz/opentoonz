...
class ino_level_auto : public TStandardRasterFx
{
  ...
  public:
	ino_level_auto()
		: m_in_min_shift(0.0 * ino::param_range()), m_in_max_shift(0.0 * ino::param_range()),
		  m_out_min(0.0 * ino::param_range()), m_out_max(1.0 * ino::param_range()),
		  m_gamma(1.0 * ino::param_range())
	{
		...
		this->m_in_min_shift->setValueRange(-1.0 * ino::param_range(), 1.0 * ino::param_range());
		this->m_in_max_shift->setValueRange(-1.0 * ino::param_range(), 1.0 * ino::param_range());
		this->m_out_min->setValueRange(0.0 * ino::param_range(), 1.0 * ino::param_range());
		this->m_out_max->setValueRange(0.0 * ino::param_range(), 1.0 * ino::param_range());
...
namespace
{
void fx_(TRasterP in_ras, bool *act_sw, double *in_min_shift, double *in_max_shift, double *out_min,
		 double *out_max, double *gamma, const int camera_x, const int camera_y, const int camera_w,
		 const int camera_h)
{
	...
	igs::level_auto_in_camera::change(
		...
		ino::channels(), ino::bits(in_ras)
							 ...
							 ,
		act_sw, in_min_shift, in_max_shift, out_min, out_max, gamma, camera_x, camera_y, camera_w,
		camera_h);
...
//------------------------------------------------------------
void ino_level_auto::doCompute(TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	/* ------ 接続していなければ処理しない -------------------- */
	if (!this->m_input.isConnected()) {
	...
	if (log_sw) {
		...
		os << "params  act true"
		   << "  in_min_shift " << in_min_sft[0] << "  in_max_shift " << in_max_sft[0]
		   << "  out_min " << out_min[0] << "  out_max " << out_max[0] << "  gamma " << gamma[0]
		   << "  frame " << frame << "  pixbits " << ino::pixel_bits(tile.getRaster())
		   << "  tile.m_pos " << tile.m_pos << "  tile_getLx " << tile.getRaster()->getLx()
		   << "  y " << tile.getRaster()->getLy() << "  rend_sets.m_cameraBox "
		   << rend_sets.m_cameraBox << "  rend_sets.m_affine " << rend_sets.m_affine
		   << "  camera x " << camera_x << "  y " << camera_y << "  w " << camera_w << "  h "
		   << camera_h;
	}
	/* ------ fx処理 ------------------------------------------ */
	try {
		tile.getRaster()->lock();
		fx_(tile.getRaster(), act_sw, in_min_sft, in_max_sft, out_min, out_max, gamma, camera_x,
