...
namespace
{
double accum_by_trans_(const double src_value /* 元値(R,G,B) */
					   ,
					   ...
					   )
{
	...
	if (1 <= integer_part) {
		...
		if (0.0 < fractional_part) {
			accumulation +=
				((accumulation * transparent + src_value) - accumulation) * fractional_part;
		}
	} else { /* 整数部分がゼロ以下のとき */
		if (0.0 < fractional_part) {
	...
	return (1.0 < accumulation) ? 1.0 : ((accumulation < 0.0) ? 0.0 : accumulation);
}
}
...
void igs::density::change(unsigned char *image_array /* RGBAでなければならない */
						  ,
						  ...
						  const double density)
{
	...
	if ((std::numeric_limits<unsigned char>::digits == bits) &&
		...
		change_template_(image_array, height, width, channels, ref, ref_mode, density);
	} else if ((std::numeric_limits<unsigned short>::digits == bits) &&
			   ((std::numeric_limits<unsigned char>::digits == ref_bits) || (0 == ref_bits))) {
		change_template_(reinterpret_cast<unsigned short *>(image_array), height, width, channels,
						 ref, ref_mode, density);
	} else if ((std::numeric_limits<unsigned short>::digits == bits) &&
			   (std::numeric_limits<unsigned short>::digits == ref_bits)) {
		change_template_(reinterpret_cast<unsigned short *>(image_array), height, width, channels,
						 reinterpret_cast<const unsigned short *>(ref), ref_mode, density);
	} else if ((std::numeric_limits<unsigned short>::digits == bits) &&
			   (std::numeric_limits<unsigned short>::digits == ref_bits)) {
