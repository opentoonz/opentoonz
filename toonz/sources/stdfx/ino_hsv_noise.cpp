...
class ino_hsv_noise : public TStandardRasterFx
{
  ...
  public:
	ino_hsv_noise()
		: m_hue(0.025 * ino::param_range()), m_sat(0.0 * ino::param_range()),
		  m_val(0.035 * ino::param_range()), m_mat(0.0 * ino::param_range()), m_random_seed(1),
		  ...
		  m_anti_alias(true), m_ref_mode(new TIntEnumParam(0, "Red"))
	{
		...
		this->m_val->setValueRange(0.0 * ino::param_range(), 1.0 * ino::param_range());
		 ...
		 const bool anti_alias_sw)
{
	...
	igs::hsv_noise::change(
		...
		,
		hue_range, sat_range, val_range, alp_range, random_seed, near_blur, effective, center, type,
...
//------------------------------------------------------------
void ino_hsv_noise::doCompute(TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	/* ------ 接続していなければ処理しない -------------------- */
	if (!this->m_input.isConnected()) {
	...
	if (log_sw) {
		...
		os << "params"
		   << "  h " << hue_range << "  s " << sat_range << "  v " << val_range << "  a "
	...
	try {
		...
		fx_(tile.getRaster(), reference_tile.getRaster(), ref_mode
			...
			,
			hue_range, sat_range, val_range, mat_range, random_seed, near_blur, term_effective,
