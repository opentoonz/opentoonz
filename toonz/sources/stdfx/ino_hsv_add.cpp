  ...
  public:
	ino_hsv_add()
		: m_from_rgba(new TIntEnumParam(0, "Red")), m_offset(0.5 * ino::param_range()),
		  m_hue(0.0 * ino::param_range()), m_sat(0.0 * ino::param_range()),
		  m_val(0.25 * ino::param_range()), m_alp(0.0 * ino::param_range())
		  ...
		  m_anti_alias(true), m_ref_mode(new TIntEnumParam(0, "Red"))
	{
		...
		this->m_val->setValueRange(-1.0 * ino::param_range(), 1.0 * ino::param_range());
...
void fx_(TRasterP in_ras, const TRasterP noise_ras, const TRasterP refer_ras, const int ref_mode
		 ...
		 const double hue_scale, const double sat_scale, const double val_scale,
...
//------------------------------------------------------------
void ino_hsv_add::doCompute(TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	/* ------ 両方とも接続していなければ処理しない ------------ */
	...
	if (log_sw) {
		...
		os << "params"
		   ...
		   << offset << "  h " << hue_scale << "  s " << sat_scale << "  v " << val_scale << "  a "
