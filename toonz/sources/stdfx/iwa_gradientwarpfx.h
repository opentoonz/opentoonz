...
class Iwa_GradientWarpFx : public TStandardRasterFx
{
  ...
  protected:
	...
	void doCompute_CPU(TTile &tile, const double frame, const TRenderSettings &settings,
					   double hLength, double vLength, int margin, TDimensionI &enlargedDim,
					   float4 *source_host, float *warper_host, float4 *result_host);

	float4 getSourceVal_CPU(float4 *source_host, TDimensionI &enlargedDim, int pos_x, int pos_y);
