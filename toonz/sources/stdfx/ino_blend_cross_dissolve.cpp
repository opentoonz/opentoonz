...
class ino_blend_cross_dissolve : public TBlendForeBackRasterFx
{
  ...
  public:
	ino_blend_cross_dissolve() : m_opacity(1.0 * ino::param_range()), m_clipping_mask(false)
	{
...
void ino_blend_cross_dissolve::computeUpAndDown(TTile &tile, double frame,
												const TRenderSettings &rs, TRasterP &dn_ras,
												TRasterP &up_ras, bool upComputesWholeTile)
{
...
void ino_blend_cross_dissolve::doCompute(TTile &tile, double frame, const TRenderSettings &rs)
{
	/* ------ 画像生成 ---------------------------------------- */
