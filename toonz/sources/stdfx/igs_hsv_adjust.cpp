					  ...
					  const bool add_blend_sw)
{
	...
	if (igs::image::rgba::siz == channels) {
		...
		for (int ii = 0; ii < pixsize; ++ii, image_array += channels) {
			...
			double rr2, gg2, bb2;
			pixel_rgba_(rr1, gg1, bb1, rr2, gg2, bb2, hue_pivot, hue_scale, hue_shift, sat_pivot,
						sat_scale, sat_shift, val_pivot, val_scale, val_shift);
...
void igs::hsv_adjust::change(unsigned char *image_array, const int height, const int width,
							 ...
							 const bool add_blend_sw)
{
	if ((1.0 == hue_scale) && (0.0 == hue_shift) && (1.0 == sat_scale) && (0.0 == sat_shift) &&
	...
	if ((std::numeric_limits<unsigned char>::digits == bits) &&
		...
		change_template_(reinterpret_cast<unsigned short *>(image_array), height, width, channels,
						 ref, ref_mode, hue_pivot, hue_scale, hue_shift, sat_pivot, sat_scale,
