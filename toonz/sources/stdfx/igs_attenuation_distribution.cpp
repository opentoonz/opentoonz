...
#endif
bool inside_polygon_(double radius, int odd_diameter, double xp, double yp, int polygon_num,
					 double degree)
{
	double radian = degree * M_PI / 180.0, add_radian = 2.0 * M_PI / polygon_num, x1 = 0, y1 = 0,
		   x2, y2, xa = -odd_diameter, xb = -odd_diameter;

	for (int ii = 0; ii <= polygon_num; ++ii, radian += add_radian, x1 = x2, y1 = y2) {
...
void attenuation_distribution_(std::vector<std::vector<double>> &lens_matrix,
 ...
 */
	...
	for (int yy = 0; yy < odd_diameter; ++yy, yp += 1.0) {
		...
		for (int xx = 0; xx < odd_diameter; ++xx, xp += 1.0) {
			...
			if ((length <= radius) &&
				((polygon_num < 3) ||
				 inside_polygon_(radius, odd_diameter, xp, yp, polygon_num, degree))) { /* 影響内 */
