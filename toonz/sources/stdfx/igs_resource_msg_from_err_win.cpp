...
/*------ マルチバイト文字列 --> ワイド文字文字列 ------*/
void igs::resource::mbs_to_wcs(const std::string &mbs, std::wstring &wcs, const UINT code_page)
{
	...
	*/
	...
	wcs.resize(length);
	length = ::MultiByteToWideChar(code_page, 0, mbs.c_str(), -1, const_cast<LPWSTR>(wcs.data()),
...
/*------ ワイド文字文字列 --> マルチバイト文字列 ------*/
void igs::resource::wcs_to_mbs(const std::wstring &wcs, std::string &mbs, const UINT code_page)
{
	/* 第４引数で -1 指定により終端文字を含む大きさを返す */
	...
	mbs.resize(length);
	length = ::WideCharToMultiByte(code_page, 0, wcs.c_str(), -1, const_cast<LPSTR>(mbs.data()),
...
const std::string igs::resource::msg_from_err_(
	...
	const std::basic_string<TCHAR> &funcsig, const std::basic_string<TCHAR> &comp_type,
	const std::basic_string<TCHAR> &msc_full_ver, const std::basic_string<TCHAR> &date,
