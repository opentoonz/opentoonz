...
template <typename PIXEL> int doBlurValue(PIXEL *SRC, int SRC_WRAP, int BLUR)
{
...
template <typename PIXEL> void myOver(PIXEL &pixout, const PIXEL &pixin, const PIXEL &color)
{
	pixout = color;
}
...
template <typename PIXEL> void myAdd(PIXEL &pixout, const PIXEL &pixin, const PIXEL &color)
{
...
template <typename PIXEL> void mySub(PIXEL &pixout, const PIXEL &pixin, const PIXEL &color)
{
...
template <typename PIXEL> void myMult(PIXEL &pixout, const PIXEL &pixin, const PIXEL &color)
{
...
template <typename PIXEL> void myLighten(PIXEL &pixout, const PIXEL &pixin, const PIXEL &color)
{
...
template <typename PIXEL> void myDarken(PIXEL &pixout, const PIXEL &pixin, const PIXEL &color)
{
	pixout.r = pixin.r < color.r ? pixin.r : color.r;
	pixout.g = pixin.g < color.g ? pixin.g : color.g;
	pixout.b = pixin.b < color.b ? pixin.b : color.b;
...
void doLayerBlending(PIXEL *pixin, PIXEL *pixout, CHANNEL *pixmatte, int inLx, int outLx, int outLy,
					 int wrapIn, int wrapOut, int dx, int dy, double transp, PIXEL color,
...
void doBodyHighlight(const TRasterPT<PIXEL> rout, const TRasterPT<PIXEL> rin, TRectD rectIn,
					 int rasInLx, int rasInLy, double frame, int blur, double transp, PIXEL color,
					 TPointD point, bool invert,
					 void (*pixelOp)(PIXEL &, const PIXEL &, const PIXEL &), TRasterFxPort &m_input,
					 const TRenderSettings &ri)
{
	...
	doLayerBlending(src_buf, dst_buf, mattepix, inLx, outLx, outLy, inWrap, outWrap, dx, dy, transp,
					color, pixelOp);
...
class BodyHighLightFx : public TStandardRasterFx
{
  ...
  public:
	BodyHighLightFx()
		: m_point(TPointD(10.0, 10.0)), m_mode(new TIntEnumParam(OVER, "Over")),
		  m_transparency(0.5), m_blur(2.0), m_color(TPixel32::White), m_invert(false)
	{
...
void BodyHighLightFx::doCompute(TTile &tile, double frame, const TRenderSettings &ri)
{
	...
	if (raster32)
		doBodyHighlight<TPixel32>(raster32, rin, rectIn, rasInLx, rasInLy, frame, blur, transp,
								  color, convert(point), invert, pixelOp32, m_input, ri);
	else {
		TRaster64P rin = tileIn.getRaster();
		if (raster64)
			doBodyHighlight<TPixel64>(raster64, rin, rectIn, rasInLx, rasInLy, frame, blur, transp,
									  toPixel64(color), convert(point), invert, pixelOp64, m_input,
									  ri);
		else
