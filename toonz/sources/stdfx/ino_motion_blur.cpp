...
class ino_motion_blur : public TStandardRasterFx
{
  ...
  public:
	ino_motion_blur()
		: m_depend_move(new TIntEnumParam(0, "P1 -> P2")), m_x1(0.0), m_y1(0.0), m_x2(1.0),
		  m_y2(1.0), m_scale(1.0 * ino::param_range()), m_curve(1.0 * ino::param_range()),
		  m_zanzo_length(0.0), m_zanzo_power(1.0 * ino::param_range()), m_alpha_rendering(true)
	// , m_spread(true)
	{
		...
		this->m_zanzo_power->setValueRange(0.0 * ino::param_range(), 1.0 * ino::param_range());

		this->getAttributes()->setIsSpeedAware(true);
	}
	//------------------------------------------------------------
	void get_render_vector(const double frame, const TAffine affine, TPointD &rend_vect)
	{
	...
	int get_render_margin(const double frame, const TAffine affine)
	{
		...
		return static_cast<int>(
			ceil((fabs(rend_vect.x) < fabs(rend_vect.y)) ? fabs(rend_vect.y) : fabs(rend_vect.x)) +
			0.5);
	}
...
void arr_to_ras_(const U *arr, const int channels, TRasterPT<T> ras, const int margin)
{
...
void vec_to_ras_(unsigned char *vec, const int channels, TRasterP ras, const int margin)
{
	if ((TRaster32P)ras) {
		arr_to_ras_<unsigned char, TPixel32>(vec, channels, ras, margin);
	} else if ((TRaster64P)ras) {
		arr_to_ras_<unsigned short, TPixel64>(reinterpret_cast<unsigned short *>(vec), channels,
											  ras, margin);
	}
}
void fx_(const TRasterP in_ras // with margin
		 ...
		 ,
		 const double x_vector, const double y_vector, const double scale, const double curve,
		 const int zanzo_length, const double zanzo_power, const bool alp_rend_sw)
{
	/******
...
******/
	...
	igs::motion_blur::convert(in_ras->getRawData() // BGRA
							  //&in_vec.at(0) // RGBA
							  ...
							  in_ras->getLy(), in_ras->getLx(), ino::channels(), ino::bits(in_ras)
																					 ...
																					 ,
							  x_vector, y_vector, scale, curve, zanzo_length, zanzo_power,
							  alp_rend_sw);
...
//------------------------------------------------------------
void ino_motion_blur::doCompute(TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	/* ------ 接続していなければ処理しない -------------------- */
	...
	const int zanzo_length = this->m_zanzo_length->getValue(frame) * ino::pixel_per_mm() *
							 sqrt(fabs(rend_sets.m_affine.det())) /
							 ((rend_sets.m_shrinkX + rend_sets.m_shrinkY) / 2.0);
	const double zanzo_power = this->m_zanzo_power->getValue(frame) / ino::param_range();
	...
	if (log_sw) {
		...
		os << "params"
		   << "  depend_move_num " << depend_move << "  render_vector x " << rend_vect.x << "  y "
		   << rend_vect.y << "  margin " << margin << "  scale " << scale << "  curve " << curve
		   << "  z_len " << zanzo_length << "  z_pow " << zanzo_power << "  alp_rend_sw "
		   << alp_rend_sw << "   tile w " << tile.getRaster()->getLx() << "  h "
		   << tile.getRaster()->getLy() << "  pixbits " << ino::pixel_bits(tile.getRaster())
		   << "   frame " << frame << "   rand_sets affine_det " << rend_sets.m_affine.det()
		   << "  a11 " << rend_sets.m_affine.a11 << "  a12 " << rend_sets.m_affine.a12 << "  a13 "
		   << rend_sets.m_affine.a13 << "  a21 " << rend_sets.m_affine.a21 << "  a22 "
		   << rend_sets.m_affine.a22 << "  a23 " << rend_sets.m_affine.a23 << "  x "
