...
bool Iwa_MotionBlurCompFx::setSourceRaster(const RASTER srcRas, float4 *dstMem, TDimensionI dim,
										   PremultiTypes type)
{
...
------------------------------------------------------------*/
template <typename RASTER, typename PIXEL>
void Iwa_MotionBlurCompFx::setOutputRaster(float4 *srcMem, const RASTER dstRas, TDimensionI dim,
										   int2 margin)
{
	int out_j = 0;
...
void Iwa_MotionBlurCompFx::makeMotionBlurFilter_CPU(float *filter_p, TDimensionI &filterDim,
	...
	for (int fily = 0; fily < filterDim.ly; fily++) {
		for (int filx = 0; filx < filterDim.lx; filx++, current_fil_p++) {
			...
			for (int v = 0; v < pointAmount - 1; v++) {
				...
				/*- 範囲内に無ければcontinue -*/
				if (pos.x < tmin(p0.x, p1.x) - 1.0f || pos.x > tmax(p0.x, p1.x) + 1.0f ||
					pos.y < tmin(p0.y, p1.y) - 1.0f || pos.y > tmax(p0.y, p1.y) + 1.0f)
					continue;
				...
				if (dot <= 0.0f) {
					...
					/*- P1より先にある場合 -*/
						...
						dist2 =
							vec_p1_sample.x * vec_p1_sample.x + vec_p1_sample.y * vec_p1_sample.y;
						framePosRatio = 1.0f;
					}
				}
			...
			for (int yy = 0; yy < 16; yy++) {
				...
				for (int xx = 0; xx < 16; xx++) {
					...
					float dot = vec_np0_sub.x * vec_np0_np1.x + vec_np0_sub.y * vec_np0_np1.y;
					/*- 距離の2乗を求める -*/
					float dist2;
					/*- P0より手前にある場合 -*/
					if (dot <= 0.0f)
						...
						/*-  P1より先にある場合 -*/
							...
							dist2 = vec_np1_sub.x * vec_np1_sub.x + vec_np1_sub.y * vec_np1_sub.y;
						}
					}
...
void Iwa_MotionBlurCompFx::makeZanzoFilter_CPU(float *filter_p, TDimensionI &filterDim,
											   int marginLeft, int marginBottom,
											   float4 *pointsTable, int pointAmount,
...
void Iwa_MotionBlurCompFx::convertRGBtoExposure_CPU(float4 *in_tile_p, TDimensionI &dim,
...
void Iwa_MotionBlurCompFx::applyBlurFilter_CPU(float4 *in_tile_p, float4 *out_tile_p,
											   TDimensionI &enlargedDim, float *filter_p,
...
void Iwa_MotionBlurCompFx::convertExposureToRGB_CPU(float4 *out_tile_p, TDimensionI &dim,
													float hardness)
{
	...
	for (int i = 0; i < dim.lx * dim.ly; i++, cur_tile_p++) {
		...
		/*- クランプ -*/
		cur_tile_p->x =
			(cur_tile_p->x > 1.0f) ? 1.0f : ((cur_tile_p->x < 0.0f) ? 0.0f : cur_tile_p->x);
		cur_tile_p->y =
			(cur_tile_p->y > 1.0f) ? 1.0f : ((cur_tile_p->y < 0.0f) ? 0.0f : cur_tile_p->y);
		cur_tile_p->z =
			(cur_tile_p->z > 1.0f) ? 1.0f : ((cur_tile_p->z < 0.0f) ? 0.0f : cur_tile_p->z);
	}
}
...
------------------------------------------------------------*/
void Iwa_MotionBlurCompFx::composeWithNoMotion(TTile &tile, double frame,
											   const TRenderSettings &settings)
{
	...
	TTile fore_tile;
	m_input->allocateAndCompute(fore_tile, tile.m_pos, tile.getRaster()->getSize(),
														 ...
														 float hardness)
{
	...
	if (backRas32)
		bgIsPremultiplied =
			setSourceRaster<TRaster32P, TPixel32>(backRas32, background_host, dimOut);
	else if (backRas64)
		bgIsPremultiplied =
			setSourceRaster<TRaster64P, TPixel64>(backRas64, background_host, dimOut);
...
Iwa_MotionBlurCompFx::Iwa_MotionBlurCompFx()
	: m_hardness(0.3)
	  /*- 左右をぼかすためのパラメータ -*/
	  ,
	  m_startValue(1.0), m_startCurve(1.0), m_endValue(1.0), m_endCurve(1.0), m_zanzoMode(false),
...
void Iwa_MotionBlurCompFx::doCompute(TTile &tile, double frame, const TRenderSettings &settings)
{
	/*- 接続していない場合は処理しない -*/
	...
	TTile back_Tile;
	if (m_background.isConnected()) {
		m_background->allocateAndCompute(back_Tile, tile.m_pos, tile.getRaster()->getSize(),
										 tile.getRaster(), frame, settings);
	}
	...
	doCompute_CPU(tile, frame, settings, pointsTable, pointAmount, hardness, shutterStart,
				  shutterEnd, traceResolution, startValue, startCurve, endValue, endCurve,
				  marginLeft, marginRight, marginTop, marginBottom, enlargedDimIn, enlarge_tile,
				  dimOut, filterDim, back_Tile);
}

//------------------------------------------------------------

void Iwa_MotionBlurCompFx::doCompute_CPU(
	TTile &tile, double frame, const TRenderSettings &settings, float4 *pointsTable,
	int pointAmount, double hardness, double shutterStart, double shutterEnd, int traceResolution,
	float startValue, float startCurve, float endValue, float endCurve, int marginLeft,
	int marginRight, int marginTop, int marginBottom, TDimensionI &enlargedDimIn,
	TTile &enlarge_tile, TDimensionI &dimOut, TDimensionI &filterDim, TTile &back_tile)
{
	/*- 処理を行うメモリ -*/
	...
	if (ras32)
		sourceIsPremultiplied = setSourceRaster<TRaster32P, TPixel32>(
			ras32, in_tile_p, enlargedDimIn, (PremultiTypes)m_premultiType->getValue());
	else if (ras64)
		sourceIsPremultiplied = setSourceRaster<TRaster64P, TPixel64>(
			ras64, in_tile_p, enlargedDimIn, (PremultiTypes)m_premultiType->getValue());

	/*- 残像モードがオフのとき -*/
	if (!m_zanzoMode->getValue()) {
		/*- フィルタをつくり、正規化する -*/
		makeMotionBlurFilter_CPU(filter_p, filterDim, marginLeft, marginBottom, pointsTable,
								 pointAmount, startValue, startCurve, endValue, endCurve);
	}
	/*- 残像モードがオンのとき -*/
	else {
		/*- 残像フィルタをつくる/正規化する -*/
		makeZanzoFilter_CPU(filter_p, filterDim, marginLeft, marginBottom, pointsTable, pointAmount,
							startValue, startCurve, endValue, endCurve);
	}
	...
	/*- 露光値をフィルタリングしてぼかす -*/
	applyBlurFilter_CPU(in_tile_p, out_tile_p, enlargedDimIn, filter_p, filterDim, marginLeft,
						marginBottom, marginRight, marginTop, dimOut);
	/*- メモリ解放 -*/
	in_tile_ras->unlock();
	filter_ras->unlock();
	...
	if (m_background.isConnected()) {
		composeBackgroundExposure_CPU(out_tile_p, enlargedDimIn, marginRight, marginTop, back_tile,
									  dimOut, (float)hardness);
	}
	/*- 露光値をdepremultipy→RGB値(０〜１)に戻す→premultiply -*/
...
bool Iwa_MotionBlurCompFx::doGetBBox(double frame, TRectD &bBox, const TRenderSettings &info)
{
	if (!m_input.isConnected() && !m_background.isConnected()) {
		bBox = TRectD();
		return false;
...
bool Iwa_MotionBlurCompFx::canHandle(const TRenderSettings &info, double frame)
{
	return true;
}
