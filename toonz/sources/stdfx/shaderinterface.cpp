...
void ShaderInterface::Parameter::saveData(TOStream &os)
{
	...
	case BOOL:
		...
		CASE IVEC3 : os << m_default.m_ivec3[0] << m_default.m_ivec3[1] << m_default.m_ivec3[2];

		CASE IVEC4 : os << m_default.m_ivec4[0] << m_default.m_ivec4[1] << m_default.m_ivec4[2]
						<< m_default.m_ivec4[3];
	...
	case FLOAT:
		...
		CASE IVEC2 : os << m_range[0].m_ivec2[0] << m_range[1].m_ivec2[0] << m_range[0].m_ivec2[1]
						<< m_range[1].m_ivec2[1];

		CASE IVEC3 : os << m_range[0].m_ivec3[0] << m_range[1].m_ivec3[0] << m_range[0].m_ivec3[1]
						<< m_range[1].m_ivec3[1]
						<< m_range[0].m_ivec3[2]
						<< m_range[1].m_ivec3[2];

		CASE IVEC4 : os << m_range[0].m_ivec4[0] << m_range[1].m_ivec4[0] << m_range[0].m_ivec4[1]
						<< m_range[1].m_ivec4[1]
						<< m_range[0].m_ivec4[2]
						<< m_range[1].m_ivec4[2]
						<< m_range[0].m_ivec4[3]
...
void ShaderInterface::Parameter::loadData(TIStream &is)
{
	...
	case BOOL:
		...
		CASE VEC4:
		{
			m_default.m_vec4[0] = m_default.m_vec4[1] = m_default.m_vec4[1] = m_default.m_vec4[1] =
				0.0;
		...
		CASE RGBA:
		{
			m_default.m_rgba[0] = m_default.m_rgba[1] = m_default.m_rgba[2] = m_default.m_rgba[3] =
				255;
		}

		CASE RGB: { m_default.m_rgb[0] = m_default.m_rgb[1] = m_default.m_rgb[2] = 255; }
	...
	while (is.openChild(tagName)) {
		if (tagName == l_names[CONCEPT])
			...
			case BOOL: {
				...
				CASE IVEC3 : is >> m_default.m_ivec3[0] >> m_default.m_ivec3[1] >>
							 m_default.m_ivec3[2];

				CASE IVEC4 : is >> m_default.m_ivec4[0] >> m_default.m_ivec4[1] >>
			...
			case FLOAT: {
				...
				CASE IVEC2 : is >> m_range[0].m_ivec2[0] >> m_range[1].m_ivec2[0] >>
							 m_range[0].m_ivec2[1] >> m_range[1].m_ivec2[1];

				CASE IVEC3 : is >> m_range[0].m_ivec3[0] >> m_range[1].m_ivec3[0] >>
							 m_range[0].m_ivec3[1] >> m_range[1].m_ivec3[1] >>
							 m_range[0].m_ivec3[2] >> m_range[1].m_ivec3[2];

				CASE IVEC4 : is >> m_range[0].m_ivec4[0] >> m_range[1].m_ivec4[0] >>
