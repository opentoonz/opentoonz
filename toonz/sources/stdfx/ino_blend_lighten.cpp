...
class ino_blend_lighten : public TBlendForeBackRasterFx
{
  ...
  public:
	ino_blend_lighten() : m_opacity(1.0 * ino::param_range()), m_clipping_mask(false)
	{
...
void ino_blend_lighten::computeUpAndDown(TTile &tile, double frame, const TRenderSettings &rs,
...
void ino_blend_lighten::dryComputeUpAndDown(TRectD &rect, double frame, const TRenderSettings &rs,
											bool upComputesWholeTile)
{
	const bool up_is =
		(this->m_up.isConnected() && this->m_up.getFx()->getTimeRegion().contains(frame));
...
void ino_blend_lighten::doCompute(TTile &tile, double frame, const TRenderSettings &rs)
{
	/* ------ 画像生成 ---------------------------------------- */
