...
template <class RT>
void blur_1st_hori_(const double **in_plane_with_margin // &(std::vector<double *>).at(0)
					,
					...
					const int ref_channels, const double real_radius, const double sigma)
{
	...
	for (int yo = 0; yo < height_with_margin; ++yo) {
		...
		for (int xx = 0, xo = int_radius; xx < width_no_margin; ++xx, ++xo) {
			...
			if (ref != 0) {
				const double read_r =
					real_radius * igs::color::ref_value(ref_hori, ref_channels, r_max, ref_mode);
				ref_hori += ref_channels;

				if (read_r != before_real_radius) {
					gauss_distribution_1d_(brush_sequence, brush_diameter,
										   igs::gaussian_blur_hv::int_radius(read_r), read_r,
										   sigma);
...
template <class RT>
void blur_2nd_vert_(const double **in_plane_with_margin // &(std::vector<double *>).at(0)
					,
					const int height_with_margin, const int width_with_margin,
					double *brush_sequence // &(std::vector<double *>).at(0)
					,
					...
					const int ref_channels, const double real_radius, const double sigma)
{
	...
	for (int xx = 0, xo = int_radius; xx < width_no_margin; ++xx, ++xo) {
		...
		for (int yy = 0, yo = int_radius; yy < height_no_margin; ++yy, ++yo) {
			...
			if (ref != 0) {
				const double read_r =
					real_radius * igs::color::ref_value(ref_vert, ref_channels, r_max, ref_mode);
				ref_vert += width_no_margin * ref_channels;
...
template <class T>
void get_(const T *in, const int height, const int width, const int channels, const int current_ch,
...
template <class T>
void put_margin_(const double **in_with_margin // &(std::vector<double *>).at(0)
				 ,
				 const int height_with_margin, const int width_with_margin, const int channels,
				 const int current_ch, const int margin, T *out_no_margin)
{
	out_no_margin += current_ch;
	const double maxi = static_cast<double>(std::numeric_limits<T>::max()) + 0.999999;
...
template <class T>
bool diff_between_channel_(const T *in, const int height, const int width, const int channels,
...
void convert_hv_(const IT *in_with_margin, IT *out_no_margin, const int height_with_margin,
				 const int width_with_margin, const int channels
				 ...
				 const double real_radius, const double sigma)
{
	...
	for (int cc = 0; cc < channels; ++cc) {
		if (0 < cc) { /* 2番目のチャンネル以後 */
			/* 1つ前のチャンネルと違いを調べる */
			diff_sw = diff_between_channel_(in_with_margin, height_with_margin, width_with_margin,
											channels, cc - 1, cc);
		}
		/* 一つ前と同じ画像なら処理せず使い回して高速化する */
		if (diff_sw) {
			get_(in_with_margin, height_with_margin, width_with_margin, channels, cc, buffer_inn);
			blur_1st_hori_((const double **)(buffer_inn), height_with_margin, width_with_margin,
						   filter, int_radius, buffer_out

						   ,
						   ref, ref_mode, channels, real_radius, sigma);
			blur_2nd_vert_((const double **)(buffer_out), height_with_margin, width_with_margin,
						   filter, int_radius, buffer_inn

						   ,
						   ref, ref_mode, channels, real_radius, sigma);
		}

		put_margin_((const double **)(buffer_inn), height_with_margin, width_with_margin, channels,
					cc, int_radius, out_no_margin);
	}
}
}
...
const int igs::gaussian_blur_hv::buffer_bytes(const int height_with_margin,
											  const int width_with_margin, const int int_radius)
{
	...
	)
{
	...
	/* filter生成 */
	gauss_distribution_1d_(&filter_buf.at(0), int_diameter, int_radius, real_radius, sigma);
	...
	if ((std::numeric_limits<unsigned char>::digits == bits) &&
		...
		convert_hv_(static_cast<const unsigned short *>(in_with_margin),
					static_cast<unsigned short *>(out_no_margin), height_with_margin,
					width_with_margin, channels, &filter_buf.at(0), int_radius,
					&in_plane_with_margin_dp.at(0), &out_plane_with_margin_dp.at(0)
