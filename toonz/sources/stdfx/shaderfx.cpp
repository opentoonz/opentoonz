	...
	ShadingContext::Support touchSupport()
	{
		...
		if (!sentMsg) {
			...
			case ShadingContext::NO_SHADERS:
				DVGui::warning(
					QGLShaderProgram::tr("This system configuration does not support OpenGL Shader "
...
void ShaderFx::initialize()
{
	...
	for (p = 0; p != pCount; ++p) {
		...
		case ShaderInterface::VEC2: {
			TPointParamP param(TPointD(siParam.m_default.m_vec2[0], siParam.m_default.m_vec2[1]));
		...
		case ShaderInterface::RGBA: {
			TPixelParamP param(TPixel32(siParam.m_default.m_rgba[0], siParam.m_default.m_rgba[1],
