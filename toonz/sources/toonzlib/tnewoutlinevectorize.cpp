...
template <typename Pix> Pix transparent();

template <> inline TPixel32 transparent<TPixel32>()
{
	return TPixel32::Transparent;
}
template <> inline TPixelGR16 transparent<TPixelGR16>()
{
...
class Edge : public tcg::Edge
{
  ...
  public:
	Edge() : tcg::Edge(), m_s(0) {}
	Edge(int v1, const TPoint &dir1, int v2, const TPoint &dir2) : tcg::Edge(v1, v2), m_s(0)
	{
...
struct SumsBuilder {
	...
	SumsBuilder(const std::vector<double> &sums_x, const std::vector<double> &sums_y,
		...
		: m_sums_x(sums_x), m_sums_y(sums_y), m_sums2_x(sums2_x), m_sums2_y(sums2_y),
		  m_sums_xy(sums_xy)
	{
	}
...
void adjustVertex(const TPointD &origin, TPointD &point, P1_type p0, Sums &sums0, int n0,
				  P2_type p1, Sums &sums1, int n1)
{
	// Find the 2 best-fit lines
	TPointD v0, v1;

	tcg::point_ops::bestFit(p0, v0, sums0.m_sums_x, sums0.m_sums_y, sums0.m_sums2_x,
							sums0.m_sums2_y, sums0.m_sums_xy, n0);

	tcg::point_ops::bestFit(p1, v1, sums1.m_sums_x, sums1.m_sums_y, sums1.m_sums2_x,
							sums1.m_sums2_y, sums1.m_sums_xy, n1);
...
void adjustVertices(const TPointD &origin, std::vector<TPointD> &points,
					const std::vector<int> &indices, const std::vector<double> &sums_x,
					const std::vector<double> &sums_y, const std::vector<double> &sums2_x,
					const std::vector<double> &sums2_y, const std::vector<double> &sums_xy)
{
	...
	if (points.front() == points.back()) {
		prev0 = indices[last - 1] - 1, prev1 = indices[last], next0 = indices[0] - 1,
		next1 = indices[1];

		sumsBuilder.build(sums0, prev0, prev1);
		sumsBuilder.build(sums1, next0, next1);

		adjustVertex<TPointD &, TPointD &>(origin, points[0], a0, sums0, prev1 - prev0, a1, sums1,
										   next1 - next0);
		...
		for (i = 1; i < last; ++i) {
			...
			adjustVertex<TPointD &, TPointD &>(origin, points[i], a0, sums0, prev1 - prev0, a1,
											   sums1, next1 - next0);
		}
	} else {
		...
		adjustVertex<const TPointD &, TPointD &>(origin, points[1], a0, sums0, prev1 - prev0 + 1,
		...
		adjustVertex<const TPointD &, TPointD &>(origin, points[1], a1, sums1, next1 - next0, a0,
												 sums0, prev1 - prev0);
	}
}
...
template <typename RanIt> class PolylineReader
{
  ...
  public:
	PolylineReader(TVectorImageP vi, const NewOutlineConfiguration &conf)
		: m_vi(vi), m_conf(conf), m_adherenceTol(2.0 * (1.0 - m_conf.m_adherenceTol)),
		  m_angleTol(cos(TConsts::pi * m_conf.m_angleTol)), m_relativeTol(conf.m_relativeTol),
...
template <typename Pix> class BordersReader : public TRop::borders::BordersReader
{
  public:
	...
	typedef
		typename tcg::hash<const TStroke *, stroke_colors_type, StrokeHashType> stroke_colors_hash;
  ...
  public:
	BordersReader(const TRasterPT<pixel_type> &ras, LocalMesh *mesh, TVectorImageP vi,
		...
		: m_mesh(mesh), m_polylineReader(vi, conf), m_loopReader(vi, conf), m_vi(vi),
		  m_vHash(&hashPoint), m_scHash(&hashStroke), m_ras(ras), m_lx(ras->getLx()),
		  m_ly(ras->getLy()), m_wrap(ras->getWrap())
	{
	}

	void openContainer(const TPoint &pos, const TPoint &dir, const pixel_type &innerColor,
...
template <typename Pix> int ::BordersReader<Pix>::surroundingEdges()
{
...
template <typename Pix> int ::BordersReader<Pix>::touchVertex(const TPoint &pos)
{
...
void ::BordersReader<Pix>::touchEdge(int v0, const TPoint &d0, int nd0, int v1, const TPoint &d1,
									 int nd1)
{
	...
	if (it == end) {
		...
		if (m_points.size() == 2) {
			...
			if (m_points.front() == m_points.back()) {
				...
				RasterEdgeEvaluator<point_iterator> eval(m_points.begin(), m_points.end(), 1.0,
...
void ::BordersReader<Pix>::addElement(const TPoint &pos, const TPoint &dir, const Pix &outerColor)
{
	...
	TPoint oppDir =
		horizontal ? TPoint((pos.x > m_pos.x) ? -1 : 1, 0) : TPoint(0, (pos.y > m_pos.y) ? -1 : 1);
...
template <typename Pix> void ::BordersReader<Pix>::closeContainer()
{
...
template <typename RanIt> void ::PolylineReader<RanIt>::openContainer(const RanIt &it)
{
	const TPoint &p = *it;
...
template <typename RanIt> void ::PolylineReader<RanIt>::addElement(const RanIt &it)
{
	const TPoint &p = *it;
	m_points.push_back(TPointD(p.x, p.y));
	m_indices.push_back(it - m_eval->begin());
...
template <typename RanIt> void ::PolylineReader<RanIt>::closeContainer()
{
	if (!m_indices.empty()) {
		const TPoint &origI(*m_eval->begin());
		TPointD origin(origI.x, origI.y);

		::adjustVertices(origin, m_points, m_indices, m_eval->sums_x(), m_eval->sums_y(),
						 m_eval->sums2_x(), m_eval->sums2_y(), m_eval->sums_xy());
	}

	std::vector<TThickPoint> cps;
	polylineToQuadratics(m_points, cps, m_adherenceTol, m_angleTol, m_relativeTol, 0.75,
						 m_mergeTol);
	...
	for (y = 0; y < ly; ++y) {
		...
		for (pix = line; pix < lineEnd; ++pix)
			*pix = (*pix == transparentColor)
					   ...
					   : palette->getStyle(palette->getClosestStyle(*pix))->getMainColor();
	}
}
...
TVectorImageP VectorizerCore::newOutlineVectorize(const TImageP &image,
												  const NewOutlineConfiguration &conf,
												  TPalette *palette)
{
	...
	// Deal with palette (observe that if image is colormap, the input palette is directly copied to
	// output)
	if (ri)
