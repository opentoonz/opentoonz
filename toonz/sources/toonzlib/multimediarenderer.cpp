...
class MultimediaRenderer::Imp : public MovieRenderer::Listener, public TSmartObject
{
  public:
	ToonzScene *m_scene;
	TFilePath m_fp;
	...
	Imp(ToonzScene *scene, const TFilePath &moviePath, int multimediaMode, int threadCount,
		bool cacheResults);
...
MultimediaRenderer::Imp::Imp(ToonzScene *scene, const TFilePath &moviePath, int multimediaMode,
							 int threadCount, bool cacheResults)
	: m_scene(scene), m_fp(moviePath), m_threadCount(threadCount), m_cacheResults(cacheResults),
	  m_xDpi(72), m_yDpi(72), m_renderSettings(), m_listeners(), m_precomputingEnabled(true),
	  m_canceled(false), m_currentFx(0), m_currentFrame(), m_multimediaMode(multimediaMode)
{
...
void MultimediaRenderer::Imp::start()
{
	...
	for (j = 0, jt = m_framesToRender.begin(); jt != m_framesToRender.end(); ++j, ++jt)
		postFxs[j] = buildPostSceneFx(m_scene, *jt, m_renderSettings.m_shrinkX,
	...
	for (i = 0; i < count; ++i) {
		...
		// NOTE: The above pairs should not be directly added to a previously declared
		// movierenderer,
		...
		for (j = 0, jt = m_framesToRender.begin(); jt != m_framesToRender.end(); ++j, ++jt) {
			...
			fx.m_frameA = buildSceneFx(m_scene, *jt, 0, m_fxsToRender.getFx(i), transforms);

			if (m_renderSettings.m_stereoscopic) {
				m_scene->shiftCameraX(m_renderSettings.m_stereoscopicShift);
				fx.m_frameB = buildSceneFx(m_scene, *jt, 0, m_fxsToRender.getFx(i), transforms);
				m_scene->shiftCameraX(-m_renderSettings.m_stereoscopicShift / 2);
		...
		std::wstring fpName =
			...
			(fxId.empty() ? L""
						  : L"_" + fxName + (fxId == fxNameNoSpaces ? L"" : L"(" + fxId + L")"));
...
MultimediaRenderer::MultimediaRenderer(ToonzScene *scene, const TFilePath &moviePath,
									   int multimediaMode, int threadCount, bool cacheResults)
