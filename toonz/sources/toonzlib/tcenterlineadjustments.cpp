...
void organizeGraphs(SkeletonList *skeleton, VectorizerCoreGlobals &g)
{
	...
	for (currGraphPtr = skeleton->begin(); currGraphPtr != skeleton->end(); ++currGraphPtr) {
		...
		// Separate single Points - can happen only when a single node gets stored in a
		// SkeletonGraph.
		...
		for (i = 0; i < JSGraph.getNodesCount(); ++i) {
			...
			for (j = 0; j < currGraph.getNode(joint).getLinksCount(); ++j) {
				...
				while (currGraph.getNode(thisNode).degree() == 2) {
					currGraph.node(thisNode)
						.setAttribute(ORGANIZEGRAPHS_SIGN); // Sign thisNode as part of a JSGraph
...
inline void findRoads(const Sequence &s)
{
	...
	for (; curr != s.m_tail; s.next(curr, currLink)) {
		...
		lowSlope =
			fabs(s.m_graphHolder->getNode(curr).getLink(currLink)->getSlope()) < roadsMaxSlope ? 1
																							   : 0;
...
inline void JunctionArea::expandArea(unsigned int initial)
{
	...
	currJSGraph->node(initial)
		.setAttribute(JointSequenceGraph::REACHED); // Nodes absorbed gets signed
	...
	for (a = 0; a < m_jointsAbsorbed.size(); ++a) {
		...
		for (currLink = 0; currLink < currJSGraph->getNode(curr).getLinksCount(); ++currLink) {
			...
			if (s.m_graphHolder->getNode(s.m_head).getLink(s.m_headLink).getAccess()) {
				...
				iLink = s.m_headLink;
				for (; i != s.m_tail &&
					   !s.m_graphHolder->getNode(i).getLink(iLink)->hasAttribute(SkeletonArc::ROAD);
					 s.next(i, iLink))
					;
...
inline bool JunctionArea::checkShape()
{
	...
	for (a = m_enteringSequences.begin(), b = m_enteringSequences.end() - 1;
		...
		for (last = first;
			 !(*currContourFamily)[contour][last].hasAttribute(ContourNode::JR_RESERVED) &&
			 numChecked < n;
		...
		for (node = first;
			 !(*currContourFamily)[contour][node].hasAttribute(ContourNode::JR_RESERVED);
			 node = (node + 1) % n) {
			...
			result &= (fabs(cross(P - A, normalize(A1 - A))) < a->m_height * shapeDistMul &&
...
inline bool checkCircles(std::vector<double> &heights)
{
	...
	test_against_max_height:
		// Reusing cos
		cos = (sin < 0.1) ? tmax(heights[i], heights[j])
...
inline void tryConfiguration(const std::vector<unsigned int> &bounds)
{
	...
	for (i = 0, first = 0; i <= bounds.size(); first = end, ++i) {
		...
		for (j = first; j < end; ++j)
			currMeanError += sq((*currEnterings)[(*heightIndicesPtr)[j]].m_height - mean);
...
inline bool JunctionArea::makeHeights()
{
	...
	sort(heightOrderedIndices.begin(), heightOrderedIndices.end(), hLess(m_enteringSequences));
...
class EntSequenceLess
{
  public:
	...
	inline bool operator()(const EnteringSequence &a, const EnteringSequence &b)
	{
		...
		return a.m_direction.y >= 0 ? b.m_direction.y >= 0 ? a.m_direction.x > b.m_direction.x : 1
...
bool JunctionArea::calculateReconstruction()
{
	...
	sort(m_enteringSequences.begin(), m_enteringSequences.end(), EntSequenceLess());
...
void JunctionArea::apply()
{
	...
	for (a = m_enteringSequences.begin(); a != m_enteringSequences.end(); ++a) {
		// Initialize infos
		newSkeletonNode = a->m_graphHolder->newNode(T3DPointD(m_newJointPosition, a->m_height));
		...
		} else //(a->m_head == head)
		{
			...
			unsigned int newHead =
				a->m_graphHolder->newNode(T3DPointD(*a->m_graphHolder->getNode(a->m_head)));
