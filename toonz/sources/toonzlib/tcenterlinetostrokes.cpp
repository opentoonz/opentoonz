  ...
  private:
	class Length
	{
	  public:
		...
		inline bool operator<(Length sl) { return n < sl.n ? 1 : n > sl.n ? 0 : l < sl.l ? 1 : 0; }
  ...
  public:
	...
	class Length
	{
	  public:
		...
		void set_CPs(const T3DPointD &a, const T3DPointD &b, const T3DPointD &c, const T3DPointD &d,
					 const T3DPointD &e)
		{
			CPs.resize(5);
			CPs[0] = a;
...
inline void SequenceConverter::addMiddlePoints()
{
	...
	if (n == 2) {
		...
		middleAddedSequence[1] =
			(*m_graph->getNode((*inputIndices)[0]) + *m_graph->getNode((*inputIndices)[1])) * 0.5;
...
inline T3DPointD int_H(const T3DPointD &A, const T3DPointD &B, double t1, double t2)
{
	return -(0.375 * (pow(t2, 4) - pow(t1, 4))) * B +
		   (pow(t2, 3) - pow(t1, 3)) * (B * 0.6667 - A * 0.5) + (pow(t2, 2) - pow(t1, 2)) * A;
}
...
inline T3DPointD int_B1a(const T3DPointD &A, const T3DPointD &B, double t1, double t2)
{
	return -(0.5 * (pow(t2, 4) - pow(t1, 4))) * B +
		   (2.0 * ((pow(t2, 3) - pow(t1, 3)) / 3.0) * (B - A) + (pow(t2, 2) - pow(t1, 2)) * A);
}
...
bool SequenceConverter::penalty(unsigned int a, unsigned int b, Length &len)
{
	...
	p = (ellProd(CPs[0], CPs[0]) + 2 * ellProd(CPs[2], CPs[2]) + ellProd(CPs[4], CPs[4]) +
		 ellProd(CPs[0], CPs[1]) + ellProd(CPs[1], CPs[2]) + ellProd(CPs[2], CPs[3]) +
		 ellProd(CPs[3], CPs[4])) /
			5.0 +
		(2 * (ellProd(CPs[1], CPs[1]) + ellProd(CPs[3], CPs[3])) + ellProd(CPs[0], CPs[2]) +
		 ellProd(CPs[2], CPs[4])) /
			15.0;
...
inline TStroke *convert(const Sequence &s, double penalty)
{
	...
	// s.m_graphHolder->getNode(s.m_head).degree() == 2) //globals->currConfig->m_outline
