...
void fillGaps(int paint, int xc, int xd, int y, int dy,
  ...
  if (fillAutopaintGaps) {
    ...
    if (fillRight) {
      x = xc;
      fillAutoPaintGaps(r, TPoint(x, oldy), true, dy, paint, saver,
                        maxCount);
    } else if (fillLeft) {
      x = xd;
      fillAutoPaintGaps(r, TPoint(x, oldy), false, dy, paint, saver,
                        maxCount);
    }
  }
  ...
  //int xe = xc, xf = xd;
  //tone = oldTone = leftPix->getTone();
  //while (leftPix < limit && tone <= oldTone) {
  //  leftPix++;
  //  xe++;
  //  oldTone = tone;
  //  tone    = leftPix->getTone();
  //}

  //tone = oldTone = rightPix->getTone();
  //while (rightPix > pix0 && tone <= oldTone) {
  //  rightPix--;
  //  xf--;
  //  oldTone = tone;
  //  tone    = rightPix->getTone();
  //}
  //if (xe > xf) return;

  if (params.m_fillNormalGaps && xd > xc + 1) {
    bool fillMiddle = oldLeftStyle == oldRightStyle;
    if (fillMiddle) {
      int maxCount = xc - xd > 4 ? xc - xd : 5;
      fillNormalGaps(r, TPoint(xd, y), true, dy, paint, saver, maxCount);
      fillNormalGaps(r, TPoint(xc, y), false, dy, paint, saver, maxCount);
    }
  }
}
    //-----------------------------------------------------------------------------
