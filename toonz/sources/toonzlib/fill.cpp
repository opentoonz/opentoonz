    ...
    static bool hasValidNeighbors(const TRasterCM32P &r, int x, int y) {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        if (bounds.contains(TPoint(nx, ny))) {
          ...
          if (neighbor->getTone() <= selfTone) ++fourCount;
        }
      }
      for (int i = 0; i < 4; ++i) {
...
void extendFill(int paint, int paintAtClickedPos, int xc, int xd, int y, int dy,
  ...
  // extend normal fill
  {
    ...
    if (doExpendNormalFill) {
      ...
      int maxCount = (params.m_shiftFill && params.m_maxFillDepth > 6)
                         ? params.m_maxFillDepth
                         : 6;
      ...
      if (newxc > 0 && newxc < r->getLx() && r->pixels(y)[newxc].isPureInk() &&
          !r->pixels(y - dy)[newxc].isPureInk()) {
        newxc += rightIsLess ? +1 : -1;
      }

      if (newxd > 0 && newxd < r->getLx() && r->pixels(y)[newxd].isPureInk() &&
          !r->pixels(y - dy)[newxd].isPureInk()) {
        newxd += rightIsLess ? +1 : -1;
      }
...
bool fill(const TRasterCM32P &r, const FillParameters &params,
  ...
  if (paintAtClickedPos == paint)
    if (params.m_shiftFill) {
      ...
      tmp.m_styleId      = paint == 0 ? TPixelCM32::getMaxPaint() : 0;
  ...
  while (!seeds.empty()) {
    ...
    while (pix <= limit) {
      ...
      if (pix->getPaint() != paint && tone <= oldtone && tone != 0 &&
          (x > 0 ? (pix - 1)->getTone() <= pix->getTone()
                 : true) &&  // skip if pix not darker than left pix
