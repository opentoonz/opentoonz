...
double retrieveParamAtLengthWithOffset(const TStroke *stroke2change,
									   ...
									   double offset)
{
	if (!isValid(stroke2change) || !isValid(offset) || length < 0)
		return -1;
	...
	assert(strokeLength >= lengthAtW && "Position of parameter is greater than stroke length!!!");
...
bool rotateStroke(const TStroke *stroke2change, TStroke *&rotated, double &from, double &to,
				  TPointD &old_w0_pos)
{
	if (!stroke2change || !isValid(from) || !isValid(to))
		return false;
	...
	// TStroke*
	rotated = ToonzExt::rotateControlPoint(stroke2change, ToonzExt::EvenInt(0), rotateAtLength);
	if (!rotated)
		return false;
...
bool findExtremesFromActionLength(double toolActionLength, const TStroke *stroke, double w,
								  ToonzExt::Interval &out)
{
	out = ToonzExt::Interval(-1.0, -1.0);

	if (!stroke || 0.0 > w || w > 1.0)
		return false;
...
bool StrokeDeformationImpl::activate_impl(const ContextStatus *status)
{
	...
	// remove empty stroke
	{
		...
		if (isAlmostZero(stroke2manipulate_->getLength())) {
			...
			TThickPoint pnt0 = stroke2manipulate_->getControlPoint(0),
						...
						delta = TThickPoint(2.0 * TConsts::epsilon, -2.0 * TConsts::epsilon, 0.0);
	...
	if (haveEmpty) {
		const double reductionFactor = computeReductionFactor(getImplStatus()->pixelSize_, 1.0);
	...
	try {
		...
		delete deformer_;
		deformer_ = new StrokeParametricDeformer(lengthOfAction, w, stroke2manipulate_,
												 potential_->clone());
	...
	bool test = increaseControlPoints(*stroke2manipulate_, *deformer_, getImplStatus()->pixelSize_);
	/*
...
{
	assert(stroke2manipulate_ && deformer_ && "Stroke and Deformer are available");

	if (!stroke2manipulate_ || !deformer_)
		return;
...
TStroke *StrokeDeformationImpl::deactivate_impl()
{
	...
	const double reductionFactor = computeReductionFactor(getImplStatus()->pixelSize_, 3.0);
...
TPointD &StrokeDeformationImpl::oldW0()
{
	return old_w0_pos_;
}
...
ToonzExt::Interval StrokeDeformationImpl::getExtremes()
{
	...
	if (getImplStatus()->isManual_ == true) {
		findExtremesFromActionLength(getImplStatus()->lengthOfAction_,
									 getImplStatus()->stroke2change_, getImplStatus()->w_,
									 extremes);
	} else {
...
bool StrokeDeformationImpl::init(const ContextStatus *status)
{
	...
	if (!this->getLastSelectedStroke() || this->getLastSelectedStroke() != status->stroke2change_ ||
		...
		ToonzExt::findCorners(status->stroke2change_, this->getSpiresList(),
							  this->getStraightsList(), status->cornerSize_, TConsts::epsilon);
													...
													ToonzExt::Interval &extremes)
{
	...
	if (status->stroke2change_->isSelfLoop()) {
		if (extremes.first > extremes.second) {
			...
			if (!rotateStroke(status->stroke2change_, stroke2transform, w, new_w, old_w0_pos_))
				return false;
			...
			case 0:
				...
				positionOfFixPoint = retrieveParamAtLengthWithOffset(
					status->stroke2change_, status->stroke2change_->getLength() * 0.5, w);
				break;
			case 1:
				if (extremes.first != -1)
					positionOfFixPoint = retrieveParamAtLengthWithOffset(
			...
			if (!rotateStroke(status->stroke2change_, stroke2transform, w, positionOfFixPoint,
							  old_w0_pos_))
				return false;
