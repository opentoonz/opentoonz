...
/*
  ...
  Operation specifier depends on the operation. For example, mm means matrix * matrix, mv matrix *
  vector, etc..
*/
...
void sum(int n, const double *x, double *&y)
{
	...
	NOTE:
	  ...
	  incx and incy are the increments in array access - ie x[incx * i] and y[incy * j] values only
	are
...
void tlin::multiply(int rows, int cols, const double *A, const double *x, double *&y)
{
	...
	void cblas_dgemv(enum CBLAS_ORDER order,  enum CBLAS_TRANSPOSE trans,  blasint m, blasint n,
		 double alpha, double  *a, blasint lda,  double  *x, blasint incx,  double beta,  double
	*y, blasint incy);
