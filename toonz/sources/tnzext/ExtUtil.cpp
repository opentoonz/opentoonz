...
{
	if (!ToonzExt::isValid(first) || !ToonzExt::isValid(second) || !ToonzExt::isValid(w))
		return false;

	if (s) {
		if (s->isSelfLoop())
			if (first > second) {
				if ((first < w && w <= 1.0) || (0.0 <= w && w < second))
					return true;
...
/**
   ...
   */
template <class T> bool isImproper(const T *tq)
{
	TPointD v1 = tq->getP0() - tq->getP1(), v2 = tq->getP2() - tq->getP1();

	if (isAlmostZero(norm2(v1)) && isAlmostZero(norm2(v2)))
		return true;
...
/**
   ...
   */
template <class T> bool curveIsStraight(const T *tq, double &t)
{
	t = -1;
	assert(tq);
	if (!tq)
...
/**
   ...
   */
template <class T> bool corner(const T *q1, const T *q2, double tolerance)
{
	if (!q1 || !q2 || !areAlmostEqual(q1->getP2(), q2->getP0()))
		return false;
...
int detectStraightIntervals_(const TThickQuadratic *ttq, ToonzExt::Intervals &intervals,
							 double tolerance)
{
	if (!ttq)
		return 0;
	...
	if (v0xv1 == 0.0 && v0_norm2 == 0.0 && v1_norm2 == 0.0) // null value with all zero
		return 0;
...
bool mapValueInStroke(const TStroke *stroke, const TThickQuadratic *ttq, double value, double &out)
{
	assert(ttq);
...
{
	if (!ttq || !ToonzExt::isValid(stroke))
		return false;
	...
	bool check = mapValueInStroke(stroke, ttq, ttq_interval.first, stroke_interval.first);
	assert(check);
	if (!check)
		return false;
	check = mapValueInStroke(stroke, ttq, ttq_interval.second, stroke_interval.second);
...
bool addQuadraticIntervalInStroke(const TStroke *stroke, ToonzExt::Intervals &stroke_intervals,
								  const TThickQuadratic *ttq, ToonzExt::Intervals &ttq_intervals)
{
	if (!ttq || !ToonzExt::isValid(stroke))
		return false;
...
bool addQuadraticSetInStroke(const TStroke *stroke, ToonzExt::Intervals &stroke_intervals,
							 const TThickQuadratic *ttq, const std::set<double> &ttq_set)
{
	if (!ttq || !ToonzExt::isValid(stroke))
		return false;
...
{
	ToonzExt::Intervals::const_iterator cit = intervals.begin(), cit_end = intervals.end();

	while (cit != cit_end) {
		if (cit->first == value || cit->second == value)
			return true;
		++cit;
	}
...
bool isCorner(const ToonzExt::Intervals &values, double w, double tolerance)
{
	...
	// first point
	if (areAlmostEqual(prev.first, w, tolerance))
		return true;
	...
	for (int i = 1; i < size; ++i) {
		...
		if (areAlmostEqual(prev.second, curr.first) && areAlmostEqual(w, curr.first, tolerance))
			return true;
		prev = curr;
	}

	// last point
	if (areAlmostEqual(curr.second, w, tolerance))
		return true;
...
{
	if (!quad1 || !quad2 || !ToonzExt::isValid(fabs(minCos)) || !ToonzExt::isValid(fabs(minSin)))
		return false;
	...
	if (norm2(tan1) == 0.0 || norm2(tan2) == 0.0)
		return false;
...
DEBUG_EXPORT double degree2sin(int degree)
{
...
DVAPI bool ToonzExt::findNearestSpireCorners(const TStroke *stroke, double w,
											 ToonzExt::Interval &out, int cornerSize,
											 const ToonzExt::Intervals *const cl, double tolerance)
{
	if (!ToonzExt::isValid(stroke) || !ToonzExt::isValid(w))
		return false;
...
DVAPI bool ToonzExt::isASpireCorner(const TStroke *s, double w, int cornerSize,
									const ToonzExt::Intervals *const cl, double tolerance)
{
	if (!ToonzExt::isValid(s) || !ToonzExt::isValid(w))
		return false;
	...
	if (!cl) {
		if (!ToonzExt::detectSpireIntervals(s, tmpValues, cornerSize))
			return false;
		values = &tmpValues;
	}
...
DVAPI bool ToonzExt::detectStraightIntervals(const TStroke *stroke, ToonzExt::Intervals &intervals,
											 double tolerance)
{
	if (!ToonzExt::isValid(stroke))
		return false;

	assert(tolerance > 0.0 && tolerance < 1.0 && "Strange tolerance are you sure???");
	...
	std::map<int, ChunkStraights>::iterator it = arrayOfChunkIntervals.begin(),
											end = arrayOfChunkIntervals.end(), aux;
	...
	if (stroke->isSelfLoop()) {
		...
		if (areParallel(v0, vn) && areInSameDirection(v0, vn) && intervals.size() > 1) {
...
DVAPI bool ToonzExt::detectSpireIntervals(const TStroke *stroke, ToonzExt::Intervals &intervals,
										  int minDegree)
{
	if (!ToonzExt::isValid(stroke))
		return false;
...
DVAPI bool ToonzExt::findNearestStraightCorners(const TStroke *stroke, double w,
...
TStroke *ToonzExt::rotateControlPoint(const TStroke *stroke,
									  const ToonzExt::EvenInt &evenControlPoint, double atLength)
{
	if (!stroke || !stroke->isSelfLoop() || !evenControlPoint.isEven())
		return 0;
#ifdef _DEBUG
	...
	// invalid length
	if (0.0 > atLength || atLength > length)
		return 0;

	const int cpCountAtBegin = stroke->getControlPointCount();
	// invalid control point
	if (0 > controlPoint || controlPoint > cpCountAtBegin)
		return 0;
...
#ifdef _DEBUG
	{
		int count = stroke->getControlPointCount(), count2 = tmpStroke.getControlPointCount();
		int i, firstDifference = -1;
		for (i = 0; i < count; ++i) {
			if ((tp1 = stroke->getControlPoint(i)) != (tp2 = tmpStroke.getControlPoint(i))) {
				firstDifference = i;
				break;
			}
		}
		...
		for (i = 0; i < count; ++i) {
			if (i < firstDifference)
				assert((tp1 = stroke->getControlPoint(i)) == (tp2 = tmpStroke.getControlPoint(i)));
			else {
	...
	int i;
	for (i = 0; i < cpCount; ++i) {
		if (head_queue == tmpStroke.getControlPoint(i))
			break;
	}
 ...
 */
DVAPI bool ToonzExt::straightCornersDetector(const TStroke *stroke, std::vector<double> &corners)
{
	...
	if (!ToonzExt::detectStraightIntervals(stroke, intervals))
		return false;
	...
	if (stroke->isSelfLoop() && curr.second == first)
									 ...
									 std::vector<double> &corners)
{
	...
	for (UINT j = 1; j < chunkCount; j++) {
		...
		if (!isAlmostZero(quad1->getLength()) && !isAlmostZero(quad2->getLength()) &&
	...
	std::copy(internal_corners.begin(), internal_corners.end(), std::back_inserter(corners));
...
{
	if (!ToonzExt::isValid(from) || !ToonzExt::isValid(to))
		return;
...
DVAPI bool ToonzExt::replaceStroke(TStroke *old_stroke, TStroke *new_stroke, unsigned int n_,
								   TVectorImageP &vi)
{
...
DVAPI bool ToonzExt::getAllW(const TStroke *stroke, const TPointD &pnt, double &dist2,
...
DVAPI bool ToonzExt::findNearestCorners(const TStroke *stroke, double w, ToonzExt::Interval &out,
										const ToonzExt::Intervals &values, double tolerance)
{
	...
	for (int i = 1; i <= size; ++i) {
		...
		if (areAlmostEqual(w, curr.first, tolerance) && (prev.second == curr.first)) {
			if (isWGood(prev.first, w, curr.second, stroke) || (prev.first == curr.second)) {
...
DVAPI void ToonzExt::findCorners(const TStroke *stroke, ToonzExt::Intervals &corners,
								 ToonzExt::Intervals &intervals, int angle, double tolerance)
{
